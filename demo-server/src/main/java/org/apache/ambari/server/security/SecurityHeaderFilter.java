...
public class SecurityHeaderFilter implements Filter
{
    ...
    private final static Logger LOG = LoggerFactory
            .getLogger(SecurityHeaderFilter.class);
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        else
        {
            ...
            strictTransportSecurity = configuration
                    .getStrictTransportSecurityHTTPResponseHeader();
            xFrameOptionsHeader = configuration
                    .getXFrameOptionsHTTPResponseHeader();
            xXSSProtectionHeader = configuration
    ...
    public void doFilter(ServletRequest servletRequest,
        ...
        if (servletResponse instanceof HttpServletResponse)
        {
            ...
            // a value is supplied
            if (sslEnabled && (strictTransportSecurity != null) &&
                    !strictTransportSecurity.isEmpty())
            {
                httpServletResponse
                        .setHeader(STRICT_TRANSPORT_HEADER, strictTransportSecurity);
            }

            // Conditionally set the X-Frame-Options HTTP response header if a value is supplied
            if ((xFrameOptionsHeader != null) &&
                    !xFrameOptionsHeader.isEmpty())
            {
                httpServletResponse
                        .setHeader(X_FRAME_OPTIONS_HEADER, xFrameOptionsHeader);
            }

            // Conditionally set the X-XSS-Protection HTTP response header if a value is supplied
            if ((xXSSProtectionHeader != null) &&
                    !xXSSProtectionHeader.isEmpty())
            {
                httpServletResponse
                        .setHeader(X_XSS_PROTECTION_HEADER, xXSSProtectionHeader);
            }
        }
        System.out.println("berk, request = " + servletRequest);
        System.out.println("berk, request = " + servletResponse);
