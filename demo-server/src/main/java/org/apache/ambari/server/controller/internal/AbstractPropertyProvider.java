 ...
 */
public abstract class AbstractPropertyProvider extends BaseProvider
        implements PropertyProvider
{

    /**
     * The property/metric information for this provider keyed by
     * component name / property id.
     */
    private final Map<String, Map<String, PropertyInfo>> componentMetrics;

    /**
     * Regular expression for checking a property id for a metric
     * argument with methods.
     * (e.g. metrics/yarn/Queue$1.replaceAll(\",q(\\d+)=\",\"/\")
     * /AppsRunning)
     */
    private static final Pattern CHECK_FOR_METRIC_ARGUMENT_METHODS_REGEX
            = Pattern
            .compile("(\\$\\d\\.[^\\$]+\\))+");

    /**
     * Regular expression for extracting the methods from a metric
     * argument.
     * (e.g. $1.replaceAll(\",q(\\d+)=\",\"/\"))
     */
    private static final Pattern FIND_ARGUMENT_METHOD_REGEX = Pattern
            .compile(".\\w+\\(.*?\\)");

    /**
     * Regular expression for extracting the arguments for methods from
     * a metric argument.
  ...
  // ----------------------------------------------------
    ...
    /**
     * Get a map of metric / property info based on the given component
     * name and property id.
     * Note that the property id may map to multiple metrics if the
     * property id is a category.
     *
    ...
    protected Map<String, PropertyInfo> getPropertyInfoMap(
            String componentName, String propertyId) {
    ...
    protected void updatePropertyInfoMap(String componentName,
        ...
        for (Map.Entry<String, PropertyInfo> entry : componentMetricMap
                .entrySet())
        {
        ...
        if (regexEntry != null)
        {
            // in the event that a category is being requested, the
          // pattern must
    ...
    /**
     * Substitute the given value into the argument in the given
     * property id.  If there are methods attached
    ...
    protected static String substituteArgument(String propertyId,
        ...
        if (argStart > -1)
        {
            ...
            // check to see if there are any methods attached to the
          // argument
            Matcher matcher = CHECK_FOR_METRIC_ARGUMENT_METHODS_REGEX
                    .matcher(argSegment);
            if (matcher.find())
            {
                ...
                while (matcher.find())
                {
                    ...
                    int openParenIndex = argName
                            .indexOf('(', matcher.start());
                    int closeParenIndex = indexOfClosingParenthesis
                            (argName, openParenIndex);

                    String methodName = argName.substring(
                            matcher.start() + 1, openParenIndex);
                    String args = argName.substring(
                    ...
                    {
                        addArgument(args, argMatcher.start(), argMatcher
                                .end(), argList, paramTypes);
                    }

                    try
                    {
                        val = invokeArgumentMethod(val, methodName,
                                argList, paramTypes);
                    }
                    catch (Exception e)
                    {
                        throw new IllegalArgumentException(
                                "Can't apply method " + methodName +
                                        " for argument " +
        ...
        throw new IllegalArgumentException(
                "Can't substitute " + val + "  for argument " + argName +
                        " in " + propertyId);
    }
    ...
    /**
     * Extract an argument from the given string and add it to the given
     * arg and param type collections.
     */
    private static void addArgument(String args, int start, int end,
                                    List<Object> argList,
    ...
    /**
     * Invoke a method on the given argument string with the given
     * parameters.
     */
    private static String invokeArgumentMethod(String argValue,
                                               String methodName,
                                               List<Object> argList,
                                               List<Class<?>>
                                                       paramTypes)
        ...
        return (String) method.invoke(argValue, argList
                .toArray(new Object[argList.size()]));
    }
    ...
    protected void updateComponentMetricMap(
        ...
        if (!componentMetricMap.containsKey(propertyId) &&
                regexKey != null && !regexKey.equals(propertyId))
        {

            PropertyInfo propertyInfo = componentMetricMap.get(regexKey);
            if (propertyInfo != null)
            {
                List<String> regexGroups = getRegexGroups(regexKey,
                        propertyId);
                ...
                for (String regexGroup : regexGroups)
                {
                    ...
                    key = key
                            .replaceFirst(FIND_REGEX_IN_METRIC_REGEX,
                                    regexGroup);
                }
                PropertyInfo compPropertyInfo = new PropertyInfo(key,
                        propertyInfo
                        .isTemporal(), propertyInfo.isPointInTime());
                compPropertyInfo
                        .setAmsHostMetric(propertyInfo.isAmsHostMetric());
        ...
        if (propertyId != null)
        {
            for (String regexGroup : regexGroups)
            {
                ...
                propertyId = propertyId
                        .replaceFirst(FIND_REGEX_IN_METRIC_REGEX, regexGroup);
            }
        }
        return new PropertyInfo(propertyId, propertyInfo
                .isTemporal(), propertyInfo.isPointInTime());
    }
    ...
    protected boolean isSupportedPropertyId(String componentName,
                                            String propertyId) {
        Map<String, PropertyInfo> componentMetricMap = componentMetrics
                .get(componentName);

        return componentMetricMap != null &&
                (componentMetricMap.containsKey(propertyId) ||
                        checkPropertyCategory(propertyId));
    }
