...
public class AmbariManagementControllerImpl implements AmbariManagementController
{
    ...
    private final static Logger LOG = LoggerFactory
            .getLogger(AmbariManagementControllerImpl.class);
    ...
    private Cache<ClusterRequest, ClusterResponse> clusterUpdateCache = CacheBuilder
    ...
    public AmbariManagementControllerImpl(ActionManager actionManager,
        ...
        maintenanceStateHelper = injector
        ...
        if (configs != null)
        {
            ...
            mysqljdbcUrl = getAmbariServerURI(JDK_RESOURCE_LOCATION + "/" +
                    configs.getMySQLJarName());
    ...
    public String getAmbariServerURI(String path) {
        if (masterProtocol == null || masterHostname == null ||
                masterPort == null)
        {
            return null;
        }
    ...
    public void createCluster(
            ClusterRequest request) throws AmbariException {
        if (request.getClusterName() == null ||
                request.getClusterName().isEmpty() ||
                request.getClusterId() != null)
        {
            throw new IllegalArgumentException(
                    "Cluster name should be provided" +
        ...
        {
            LOG.debug(
                    "Received a createCluster request" + ", clusterName=" +
        ...
        if (request.getHostNames() != null)
        {
            clusters.mapHostsToCluster(request.getHostNames(), request
                    .getClusterName());
        }
    ...
    @Override
    public synchronized void createHostComponents(
        ...
        for (ServiceComponentHostRequest request : requests)
        {
            ...
            {
                request.setServiceName(findServiceName(cluster, request
                        .getComponentName()));
            }
            ...
            {
                hostComponentNames.put(request
                        .getClusterName(), new HashMap<String, Map<String, Set<String>>>());
            }
                                   ...
                                   .containsKey(request.getServiceName()))
            {
                hostComponentNames.get(request.getClusterName())
                                  .put(request
                                          .getServiceName(), new HashMap<String, Set<String>>());
            }
            if (!hostComponentNames.get(request.getClusterName())
                                           ...
                                           .getComponentName()))
            {
                hostComponentNames.get(request.getClusterName())
                                  .get(request.getServiceName())
                                  .put(request
                                          .getComponentName(), new HashSet<String>());
            }
            if (hostComponentNames.get(request.getClusterName())
                                  .get(request.getServiceName())
            ...
            {
                throw new IllegalArgumentException(
                        "The service[" + request.getServiceName() +
                                "] associated with the component[" +
                                request.getComponentName() +
                                "] doesn't exist for the cluster[" +
                                request.getClusterName() + "]");
            }
            ServiceComponent sc = s
                    .getServiceComponent(request.getComponentName());
            ...
            Set<Cluster> mappedClusters = clusters
                    .getClustersForHost(request.getHostname());
            ...
            for (Cluster mappedCluster : mappedClusters)
            {
                if (LOG.isDebugEnabled())
                {
                    LOG.debug("Host belongs to cluster" + ", hostname=" +
                            request.getHostname() + ", clusterName=" +
                            mappedCluster.getClusterName());
                }
                if (mappedCluster.getClusterName()
                                 .equals(request.getClusterName()))
                {
                    validCluster = true;
                    break;
                }
            ...
            if (!validCluster)
            {
                throw new ParentObjectNotFoundException(
                        "Attempted to add a host_component to a host that doesn't exist: " +
                                "clusterName=" + request.getClusterName() +
                                ", hostName=" + request.getHostname());
            }
            try
            {
                ServiceComponentHost sch = sc
                        .getServiceComponentHost(request.getHostname());
                if (sch != null)
                {
                    duplicates.add("[clusterName=" +
                            request.getClusterName() + ", hostName=" +
                            request.getHostname() +
                            ", componentName=" +
                            request.getComponentName() + ']');
                }
        ...
        {
            throw new IllegalArgumentException(
                    "Invalid arguments - updates allowed" +
    ...
    void persistServiceComponentHosts(
            Set<ServiceComponentHostRequest> requests) throws AmbariException {
        Multimap<Cluster, ServiceComponentHost> schMap = ArrayListMultimap
                .create();
        
        for (ServiceComponentHostRequest request : requests)
        {
            Cluster cluster = clusters
                    .getCluster(request.getClusterName());
            Service s = cluster.getService(request.getServiceName());
            ServiceComponent sc = s
                    .getServiceComponent(request.getComponentName());
            
            ServiceComponentHost sch = serviceComponentHostFactory
                    .createNew(sc, request.getHostname());
    ...
    private void setMonitoringServicesRestartRequired(
        ...
        for (ServiceComponentHostRequest request : requests)
        {
            ...
            Collection<String> monitoringServices = ambariMetaInfo
                    .getMonitoringServiceNames(stackId
                            .getStackName(), stackId.getStackVersion());
            ...
            for (String serviceName : monitoringServices)
            {
                if (cluster.getServices().containsKey(serviceName))
                {
                    ...
                    for (ServiceComponent sc : service
                            .getServiceComponents().values())
                    {
                        if (sc.isMasterComponent())
                        {
                            for (ServiceComponentHost sch : sc
    ...
    private void setRestartRequiredServices(Service service,
                                            String hostName) throws AmbariException {
        
        Cluster cluster = service.getCluster();
        StackId stackId = cluster.getCurrentStackVersion();
        Set<String> needRestartServices = ambariMetaInfo
                .getRestartRequiredServicesNames(stackId
                        .getStackName(), stackId.getStackVersion());
        
        if (needRestartServices.contains(service.getName()))
        {
            Map<String, ServiceComponent> m = service
            ...
            for (Entry<String, ServiceComponent> entry : m.entrySet())
            {
                ...
                {
                    Map<String, ServiceComponentHost> schMap = serviceComponent
                    ...
                    {
                        for (Entry<String, ServiceComponentHost> sch : schMap
                                .entrySet())
                        {
                            ServiceComponentHost serviceComponentHost = sch
                                    .getValue();
    ...
    @Override
    public void registerRackChange(
            String clusterName) throws AmbariException {
        Cluster cluster = clusters.getCluster(clusterName);
        StackId stackId = cluster.getCurrentStackVersion();
        
        Set<String> rackSensitiveServices = ambariMetaInfo
                .getRackSensitiveServicesNames(stackId
                        .getStackName(), stackId.getStackVersion());
        ...
        for (Service service : services.values())
        {
            ...
            {
                Map<String, ServiceComponent> serviceComponents = service
                        .getServiceComponents();
    ...
    public synchronized ConfigurationResponse createConfiguration(
        ...
        Map<String, Config> configs = cluster
                .getConfigsByType(request.getType());
        if (null == configs)
        {
        ...
        Map<String, Map<String, String>> propertiesAttributes = request
        ...
        {
            throw new AmbariException(MessageFormat
                    .format("Configuration with tag ''{0}'' exists for ''{1}''", request
                            .getVersionTag(), request.getType()));
        }
        
        handleGlobalsBackwardsCompability(request, propertiesAttributes);
        
        Config config = createConfig(cluster, request.getType(), request
                .getProperties(), request
                .getVersionTag(), propertiesAttributes);
        
        return new ConfigurationResponse(cluster.getClusterName(), config);
    }
    
    private void handleGlobalsBackwardsCompability(
        ...
        if (request.getType().equals(Configuration.GLOBAL_CONFIG_TAG))
        {
            ...
            configTypes.put(Configuration.GLOBAL_CONFIG_TAG, request
                    .getProperties());
            configHelper.moveDeprecatedGlobals(cluster
                    .getCurrentStackVersion(), configTypes, cluster
                    .getClusterName());
            
            for (Map.Entry<String, Map<String, String>> configType : configTypes
                    .entrySet())
            {
    ...
    private Config createConfig(Cluster cluster, String type,
                                Map<String, String> properties,
                                String versionTag,
                                Map<String, Map<String, String>> propertiesAttributes) {
        Config config = configFactory
                .createNew(cluster, type, properties, propertiesAttributes);
    ...
    public void createUsers(
            Set<UserRequest> requests) throws AmbariException {
        ...
        for (UserRequest request : requests)
        {
            ...
            users.createUser(request.getUsername(), request
                    .getPassword(), request.isActive(), request
                    .isAdmin(), false);
        }
    }
    ...
    public void createGroups(
            Set<GroupRequest> requests) throws AmbariException {
    ...
    public void createMembers(
            Set<MemberRequest> requests) throws AmbariException {
        for (MemberRequest request : requests)
        {
            ...
            users.addMemberToGroup(request.getGroupName(), request
                    .getUserName());
        }
    }
    ...
    public Set<MemberResponse> getMembers(
        ...
        for (MemberRequest request : requests)
        {
            LOG.debug("Received a getMembers request, " +
                    request.toString());
            final Group group = users.getGroup(request.getGroupName());
            if (null == group)
            {
            ...
            {
                for (User user : users
                        .getGroupMembers(group.getGroupName()))
                {
                    final MemberResponse response = new MemberResponse(group
                            .getGroupName(), user.getUserName());
    ...
    public synchronized void updateMembers(
        ...
        {
            if (groupName != null &&
                    !request.getGroupName().equals(groupName))
            {
        ...
        for (String user : (Collection<String>) CollectionUtils
                .subtract(currentMembers, requiredMembers))
        {
            users.removeMemberFromGroup(groupName, user);
        }
        for (String user : (Collection<String>) CollectionUtils
                .subtract(requiredMembers, currentMembers))
        {
    ...
    private Stage createNewStage(long id, Cluster cluster, long requestId,
        ...
        Stage stage = stageFactory.createNew(requestId, logDir,
                null == cluster ? null : cluster.getClusterName(),
                null == cluster ? -1L :
                        cluster.getClusterId(), requestContext, clusterHostInfo, commandParamsStage, hostParamsStage);
        stage.setStageId(id);
        return stage;
    }
    
    private Set<ClusterResponse> getClusters(
            ClusterRequest request) throws AmbariException {
        ...
        if (LOG.isDebugEnabled())
        {
            LOG.debug("Received a getClusters request" + ", clusterName=" +
                    request.getClusterName() + ", clusterId=" +
                    request.getClusterId() + ", stackInfo=" +
        ...
        if (singleCluster != null)
        {
            ...
            cr.setDesiredServiceConfigVersions(singleCluster
        ...
        for (Cluster c : allClusters.values())
        {
            if (request.getStackVersion() != null)
            {
                if (!request.getStackVersion()
                            .equals(c.getDesiredStackVersion()
                                     .getStackId()))
                {
    ...
    private Set<ServiceComponentHostResponse> getHostComponents(
        ...
        if (request.getHostname() != null)
        {
            try
            {
                if (!clusters.getClustersForHost(request.getHostname())
                             .contains(cluster))
                {
                    // case where host exists but not associated with given cluster
                    LOG.error("Host doesn't belong to cluster - " +
                            request.getHostname());
                    throw new ParentObjectNotFoundException("Parent Host resource doesn't exist", new HostNotFoundException(request
                            .getClusterName(), request.getHostname()));
                }
            }
            catch (HostNotFoundException e)
            {
        ...
        if (request.getComponentName() != null)
        {
                    ...
                    request.getServiceName().isEmpty())
            {
                ...
                String serviceName = ambariMetaInfo
                        .getComponentToService(stackId
                                .getStackName(), stackId
                                .getStackVersion(), request
                                .getComponentName());
                if (LOG.isDebugEnabled())
                {
                    LOG.debug("Looking up service name for component" +
                            ", componentName=" +
                            request.getComponentName() + ", serviceName=" +
                            serviceName + ", stackInfo=" +
                            stackId.getStackId());
                }
                if (serviceName == null || serviceName.isEmpty())
                {
                    LOG.error("Unable to find service for component {}", request
                            .getComponentName());
                    throw new ServiceComponentHostNotFoundException(cluster
                            .getClusterName(), null, request
                            .getComponentName(), request.getHostname());
                }
        ...
        if (request.getStaleConfig() != null)
        {
            ...
            staleConfig = "true"
                    .equals(request.getStaleConfig().toLowerCase());
        }
        if (request.getDesiredState() != null &&
                !request.getDesiredState().isEmpty())
        {
            ...
            {
                throw new IllegalArgumentException(
                        "Invalid arguments, invalid desired" +
                                " state, desiredState=" +
                                desiredStateToCheck);
            }
        ...
        if (!StringUtils.isEmpty(request.getState()))
        {
            ...
            {
                throw new IllegalArgumentException(
                        "Invalid arguments, invalid state, State=" +
                                request.getState());
            }
            checkState = true;
        }
        
        Map<String, Host> hosts = clusters
                .getHostsForCluster(cluster.getClusterName());
        ...
        for (Service s : services)
        {
            ...
            if (request.getComponentName() != null)
            {
                components.add(s.getServiceComponent(request
                        .getComponentName()));
            }
            else
            {
            ...
            for (ServiceComponent sc : components)
            {
                ...
                Map<String, ServiceComponentHost> serviceComponentHostMap = sc
                        .getServiceComponentHosts();
                ...
                if (request.getHostname() != null)
                {
                    try
                    {
                        ...
                        {
                            throw new ServiceComponentHostNotFoundException(cluster
                                    .getClusterName(), s.getName(), sc
                                    .getName(), request.getHostname());
                        }
                        
                        ServiceComponentHost sch = serviceComponentHostMap
                                .get(request.getHostname());
                        ...
                        if (checkDesiredState && (desiredStateToCheck !=
                                sch.getDesiredState()))
                        {
                            continue;
                        }
                        ...
                        if (request.getAdminState() != null)
                        {
                            String stringToMatch =
                                    sch.getComponentAdminState() == null ?
                                            "" :
                                            sch.getComponentAdminState()
                                               .name();
                            if (!request.getAdminState()
                                        .equals(stringToMatch))
                            {
                                continue;
                            }
                        }
                        
                        ServiceComponentHostResponse r = sch
                                .convertToResponse();
                        if (null == r || (filterBasedConfigStaleness &&
                                r.isStaleConfig() != staleConfig))
                        {
                            continue;
                        }
                        ...
                        if (host == null)
                        {
                            throw new HostNotFoundException(cluster
                                    .getClusterName(), sch.getHostName());
                        }
                        
                        r.setMaintenanceState(maintenanceStateHelper
                                .getEffectiveState(sch, host).name());
                        response.add(r);
                    }
                ...
                {
                    for (ServiceComponentHost sch : serviceComponentHostMap
                            .values())
                    {
                        if (null == sch)
                        {
    ...
    @Override
    public MaintenanceState getEffectiveMaintenanceState(
    ...
    private Set<ConfigurationResponse> getConfigurations(
        ...
        {
            throw new IllegalArgumentException(
                    "Invalid arguments, cluster name" +
                            " should not be null");
        }
        ...
        if (null != request.getType() && null != request.getVersionTag())
        {
            Config config = cluster
                    .getConfig(request.getType(), request.getVersionTag());
            if (null != config)
            {
                ConfigurationResponse response = new ConfigurationResponse(cluster
        ...
        else
        {
            ...
            if (null != request.getType())
            {
                ...
                if (null != configs)
                {
                    for (Entry<String, Config> entry : configs.entrySet())
                    {
                        ...
                        ConfigurationResponse response = new ConfigurationResponse(cluster
                                .getClusterName(), config
                                .getStackId(), request.getType(), config
                                .getTag(), entry.getValue().getVersion(),
                                includeProps ? config.getProperties() :
            ...
            else
            {
                ...
                {
                    ConfigurationResponse response = new ConfigurationResponse(cluster
                            .getClusterName(), config.getStackId(), config
                            .getType(), config.getTag(), config
                            .getVersion(),
                            includeProps ? config.getProperties() :
                                    new HashMap<String, String>(),
                            includeProps ?
    ...
    @Override
    public synchronized RequestStatusResponse updateClusters(
            Set<ClusterRequest> requests,
            Map<String, String> requestProperties) throws AmbariException {
        ...
        for (ClusterRequest request : requests)
        {
            ...
            Map<String, Object> sessionAttributes = request
                    ...
                    !sessionAttributes.isEmpty())
            {
                ...
                for (Map.Entry<String, Object> entry : sessionAttributes
                        .entrySet())
                {
                ...
                if (principal != null)
                {
                    ...
                    kerberosHelper
                            .setKDCCredentials(new KerberosCredential(principal, password, null));
                }
    ...
    private synchronized RequestStatusResponse updateCluster(
        ...
        LOG.info("Received a updateCluster request" + ", clusterId=" +
                request.getClusterId() + ", clusterName=" +
                request.getClusterName() + ", securityType=" +
                request.getSecurityType() + ", request=" + request);
        ...
        {
            String msg =
                    "Unable to set desired configs and rollback at same time, request = " +
                            request.toString();
            LOG.error(msg);
        ...
        if (request.getDesiredConfig() != null)
        {
            for (ConfigurationRequest desiredConfig : request
                    .getDesiredConfig())
            {
                Map<String, String> requestConfigProperties = desiredConfig
                        .getProperties();
                Map<String, Map<String, String>> requestConfigAttributes = desiredConfig
                        .getPropertiesAttributes();
                Config clusterConfig = cluster
                        .getDesiredConfigByType(desiredConfig.getType());
                Map<String, String> clusterConfigProperties = null;
                Map<String, Map<String, String>> clusterConfigAttributes = null;
                if (clusterConfig != null)
                {
                    clusterConfigProperties = clusterConfig
                            .getProperties();
                    clusterConfigAttributes = clusterConfig
                            .getPropertiesAttributes();
                ...
                {
                    Config existingConfig = cluster.getConfig(desiredConfig
                            .getType(), desiredConfig.getVersionTag());
                    if (existingConfig != null)
                    {
                        if (!StringUtils.equals(existingConfig
                                .getTag(), clusterConfig.getTag()))
                        {
                            isConfigurationCreationNeeded = true;
                            break;
                        }
                        ...
                        clusterConfigProperties != null)
                {
                    ...
                    {
                        for (Entry<String, String> property : requestConfigProperties
                                .entrySet())
                        {
                            if (!property.getValue()
                                         .equals(clusterConfigProperties
                                                 .get(property.getKey())))
                            {
                                isConfigurationCreationNeeded = true;
                                break;
        ...
        if (isConfigurationCreationNeeded)
        {
            ...
            for (ConfigurationRequest cr : request.getDesiredConfig())
            {
                ...
                if (null != cr.getProperties())
                {
                    ...
                    Map<String, Config> all = cluster
                            .getConfigsByType(cr.getType());
                    if (null == all ||
                            // none set
                            !all.containsKey(cr.getVersionTag()) ||
                            // tag not set
                            cr.getProperties().size() > 0)
                    {            // properties to set
                        
                        LOG.info(MessageFormat
                                .format("Applying configuration with tag ''{0}'' to cluster ''{1}''  for configuration type {2}", cr
                                        .getVersionTag(), request
                                        .getClusterName(), cr.getType()));
                        
                        cr.setClusterName(cluster.getClusterName());
                        configurationResponses
                                .add(createConfiguration(cr));
                    }
                }
                note = cr.getServiceConfigVersionNote();
                configs.add(cluster
                        .getConfig(cr.getType(), cr.getVersionTag()));
            }
            if (!configs.isEmpty())
            {
                String authName = getAuthName();
                serviceConfigVersionResponse = cluster
                        .addDesiredConfig(authName, configs, note);
                if (serviceConfigVersionResponse != null)
                {
                    Logger logger = LoggerFactory
                            .getLogger("configchange");
                    for (Config config : configs)
                    {
                        logger.info(
                                "cluster '" + request.getClusterName() +
                                        "' " + "changed by: '" + authName +
                                        "'; " + "type='" +
                                        config.getType() + "' " + "tag='" +
                                        config.getTag() + "'");
        ...
        boolean requiresHostListUpdate = request.getHostNames() != null &&
                !request.getHostNames().isEmpty();
        ...
        if (null != request.getProvisioningState())
        {
            ...
            State provisioningState = State
                    .valueOf(request.getProvisioningState());
            
            if (provisioningState != State.INIT &&
                    provisioningState != State.INSTALLED)
            {
                LOG.warn("Invalid cluster provisioning state {} cannot be set on the cluster {}", provisioningState, request
                        .getClusterName());
                
                throw new IllegalArgumentException(
                        "Invalid cluster provisioning state " +
                                provisioningState +
                                " cannot be set on cluster " +
                                request.getClusterName());
            }
            
            if (provisioningState != oldProvisioningState)
            {
                boolean isStateTransitionValid = State
                        .isValidDesiredStateTransition(oldProvisioningState, provisioningState);
                
                if (!isStateTransitionValid)
                {
                    LOG.warn("Invalid cluster provisioning 2state {} cannot be set on the cluster {} because the current state is {}", provisioningState, request
                            .getClusterName(), oldProvisioningState);
                    ...
                    throw new AmbariException("Invalid transition for" +
                            ...
                            oldProvisioningState +
                            ", newProvisioningState=" + provisioningState);
                }
            }
        ...
        {
            ServiceConfigVersionRequest serviceConfigVersionRequest = request
                    .getServiceConfigVersionRequest();
            if (StringUtils.isEmpty(serviceConfigVersionRequest
                    .getServiceName()) ||
            ...
            serviceConfigVersionResponse = cluster
                    .setServiceConfigVersion(serviceConfigVersionRequest
                            .getServiceName(), serviceConfigVersionRequest
                            .getVersion(), getAuthName(), serviceConfigVersionRequest
                            .getNote());
        }
        ...
        if (serviceConfigVersionResponse != null)
        {
            if (!configurationResponses.isEmpty())
            {
                serviceConfigVersionResponse
                        .setConfigurations(configurationResponses);
            }
            
            ClusterResponse clusterResponse = new ClusterResponse(cluster
                    .getClusterId(), cluster
                    .getClusterName(), null, null, null, null, null, null);
            
            Map<String, Collection<ServiceConfigVersionResponse>> map = new HashMap<String, Collection<ServiceConfigVersionResponse>>();
            map.put(serviceConfigVersionResponse
                    .getServiceName(), Collections
                    .singletonList(serviceConfigVersionResponse));
        ...
        if (securityType != null)
        {
            ...
            if (kerberosHelper
                    .shouldExecuteCustomOperations(securityType, requestProperties))
            {
                try
                {
                    requestStageContainer = kerberosHelper
                            .executeCustomOperations(cluster, requestProperties, requestStageContainer, kerberosHelper
                                    .getManageIdentitiesDirective(requestProperties));
                }
            ...
            {
                LOG.info("Received cluster security type change request from {} to {}", cluster
                        .getSecurityType().name(), securityType.name());
                        ...
                        (securityType == SecurityType.NONE))
                {
                    ...
                    {
                        requestStageContainer = kerberosHelper
                                .toggleKerberos(cluster, securityType, requestStageContainer, kerberosHelper
                                        .getManageIdentitiesDirective(requestProperties));
                    }
                    catch (KerberosOperationException e)
                    {
                ...
                else
                {
                    throw new IllegalArgumentException(String
                            .format("Unexpected security type encountered: %s", securityType
                                    .name()));
                }
     ...
     */
    public boolean isAttributeMapsEqual(
        ...
        if ((requestConfigAttributes != null &&
                clusterConfigAttributes == null) ||
                (requestConfigAttributes == null &&
                        clusterConfigAttributes != null) ||
                (requestConfigAttributes != null &&
                        clusterConfigAttributes != null &&
                        !requestConfigAttributes.keySet()
                                                .equals(clusterConfigAttributes
                                                        .keySet())))
        {
            return false;
        }
        else if (clusterConfigAttributes != null &&
                requestConfigAttributes != null)
        {
            for (Entry<String, Map<String, String>> ClusterEntrySet : clusterConfigAttributes
                    .entrySet())
            {
                Map<String, String> clusterMapAttributes = ClusterEntrySet
                        .getValue();
                Map<String, String> requestMapAttributes = requestConfigAttributes
                        .get(ClusterEntrySet.getKey());
                if ((requestMapAttributes != null &&
                        clusterMapAttributes == null) ||
                        (requestMapAttributes == null &&
                                clusterMapAttributes != null) ||
                        (requestMapAttributes != null &&
                                clusterMapAttributes != null &&
                                !requestMapAttributes.keySet()
                                                     .equals(clusterMapAttributes
                                                             .keySet())))
                {
                    return false;
                }
                else if (requestMapAttributes != null &&
                        clusterMapAttributes != null)
                {
                    for (Entry<String, String> requestPropertyEntrySet : requestMapAttributes
                            .entrySet())
                    {
                        String requestPropertyValue = requestPropertyEntrySet
                                .getValue();
                        String clusterPropertyValue = clusterMapAttributes
                                .get(requestPropertyEntrySet.getKey());
                        if ((requestPropertyValue != null &&
                                clusterPropertyValue == null) ||
                                (requestPropertyValue == null &&
                                        clusterPropertyValue != null) ||
                                (requestPropertyValue != null &&
                                        clusterPropertyValue != null &&
                                        !requestPropertyValue
                                                .equals(clusterPropertyValue)))
                        {
                            return false;
                        }
                    }
     ...
     */
    public void saveClusterUpdate(ClusterRequest clusterRequest,
    ...
    @Override
    public ClusterResponse getClusterUpdateResults(
    ...
    public String getJobTrackerHost(Cluster cluster) {
        try
        {
            ...
            ServiceComponent sc = svc
                    .getServiceComponent(Role.JOBTRACKER.toString());
            if (sc.getServiceComponentHosts() != null &&
                    !sc.getServiceComponentHosts().isEmpty())
            {
                return sc.getServiceComponentHosts().keySet().iterator()
                         .next();
            }
        }
    ...
    private Set<String> getServicesForSmokeTests(Cluster cluster,
        ...
        {
            for (Entry<State, List<Service>> entry : changedServices
                    .entrySet())
            {
                ...
                {
                    if (runSmokeTest &&
                            (State.INSTALLED == s.getDesiredState() &&
                                    maintenanceStateHelper
                                            .isOperationAllowed(opLvl, s)))
                    {
        ...
        for (Map<State, List<ServiceComponentHost>> stateScHostMap : changedScHosts
                .values())
        {
            for (Entry<State, List<ServiceComponentHost>> entry : stateScHostMap
                    .entrySet())
            {
                if (State.STARTED != entry.getKey())
                {
                ...
                for (ServiceComponentHost sch : entry.getValue())
                {
                    ...
                    if (!maintenanceStateHelper
                            .isOperationAllowed(opLvl, sch))
                    {
                        continue;
                    }
                    if (!changedComponentCount
                            .containsKey(sch.getServiceName()))
                    {
                        changedComponentCount
                                .put(sch.getServiceName(), new HashMap<String, Integer>());
                    }
                    ...
                    {
                        Integer i = changedComponentCount
                                .get(sch.getServiceName())
        ...
        for (Entry<String, Map<String, Integer>> entry : changedComponentCount
                .entrySet())
        {
            ...
            for (String componentName : changedComponentCount
                    .get(serviceName).keySet())
            {
                ServiceComponent sc = cluster.getService(serviceName).
                        getServiceComponent(componentName);
                StackId stackId = sc.getDesiredStackVersion();
                ComponentInfo compInfo = ambariMetaInfo
                        .getComponent(stackId.getStackName(), stackId
                                .getStackVersion(), serviceName, componentName);
    ...
    private void addClientSchForReinstall(Cluster cluster,
        ...
        {
            for (Entry<String, Map<State, List<ServiceComponentHost>>> stringMapEntry : changedScHosts
                    .entrySet())
            {
                for (State state : stringMapEntry.getValue().keySet())
                {
                    if (state == State.STARTED)
                    {
                        serviceComponentHosts
                                .addAll(stringMapEntry.getValue()
                                                      .get(state));
                    }
                }
            }
        ...
        for (String serviceName : services)
        {
            ...
            for (String component : s.getServiceComponents().keySet())
            {
                ...
                ServiceComponent sc = s.getServiceComponents()
                                       .get(component);
                if (sc.isClientComponent())
                {
                    ...
                    {
                        Host host = clusters
                                .getHost(potentialSch.getHostName());
                        // Host is alive and neither host nor SCH is in Maintenance State
                        if (!potentialSch.getHostState()
                                         .equals(HostState.HEARTBEAT_LOST) &&
                                potentialSch.getMaintenanceState() !=
                                        MaintenanceState.ON &&
                                host.getMaintenanceState(cluster
                                        .getClusterId()) ==
        ...
        {
            for (Entry<String, List<ServiceComponentHost>> stringListEntry : clientSchs
                    .entrySet())
            {
     ...
     */
    private void createHostAction(Cluster cluster, Stage stage,
        ...
        stage.addHostRoleExecutionCommand(scHost.getHost(), Role
                .valueOf(scHost
                        .getServiceComponentName()), roleCommand, event, cluster, serviceName, false, false);
        
        String componentName = scHost.getServiceComponentName();
        String hostname = scHost.getHostName();
        String osFamily = clusters.getHost(hostname).getOsFamily();
        StackId stackId = cluster.getDesiredStackVersion();
        ServiceInfo serviceInfo = ambariMetaInfo
                .getService(stackId.getStackName(), stackId
                        .getStackVersion(), serviceName);
        ...
        ExecutionCommand execCmd = stage
                .getExecutionCommandWrapper(scHost.getHostName(), scHost
                        .getServiceComponentName()).getExecutionCommand();
        ...
        if (!scHost.getHostName().equals(jobtrackerHost))
        {
            ...
            {
                configHelper
                        .applyCustomConfig(configurations, Configuration.GLOBAL_CONFIG_TAG, Configuration.RCA_ENABLED_PROPERTY, "false", false);
            }
        }
        ...
        while (it.hasNext())
        {
            ServiceInfo serviceInfoInstance = ambariMetaInfo
                    .getService(stackId.getStackName(), stackId
                            .getStackVersion(), it.next().getName());
            LOG.info(
            ...
            if (serviceInfoInstance.getServiceType() != null)
            {
                ...
                commandParams.put("dfs_type", serviceInfoInstance
                        .getServiceType());
                break;
            }
        }
        ...
        String agentDefaultCommandTimeout = configs
                .getDefaultAgentTaskTimeout(isInstallCommand);
     ...
     */
        ...
        if (serviceInfo.getSchemaVersion()
                       .equals(AmbariMetaInfo.SCHEMA_VERSION_2))
        {
            if (script != null)
            {
                commandParams.put(SCRIPT, script.getScript());
                commandParams.put(SCRIPT_TYPE, script.getScriptType()
                                                     .toString());
                
                boolean retryEnabled = false;
                Integer retryMaxTime = 0;
                if (commandParams.containsKey(CLUSTER_PHASE_PROPERTY) &&
                        (commandParams.get(CLUSTER_PHASE_PROPERTY)
                                      .equals(CLUSTER_PHASE_INITIAL_INSTALL) ||
                                commandParams.get(CLUSTER_PHASE_PROPERTY)
                                             .equals(CLUSTER_PHASE_INITIAL_START)))
                {
                    String retryEnabledStr = configHelper
                            .getValueFromDesiredConfigurations(cluster, ConfigHelper.CLUSTER_ENV, ConfigHelper.CLUSTER_ENV_RETRY_ENABLED);
                    String commandsStr = configHelper
                            .getValueFromDesiredConfigurations(cluster, ConfigHelper.CLUSTER_ENV, ConfigHelper.CLUSTER_ENV_RETRY_COMMANDS);
                    String retryMaxTimeStr = configHelper
                            .getValueFromDesiredConfigurations(cluster, ConfigHelper.CLUSTER_ENV, ConfigHelper.CLUSTER_ENV_RETRY_MAX_TIME_IN_SEC);
                    if (StringUtils.isNotEmpty(retryEnabledStr))
                    {
                        retryEnabled = Boolean.TRUE.toString()
                                                   .equals(retryEnabledStr);
                    }
                    
                    if (retryEnabled)
                    {
                        retryMaxTime = NumberUtils
                                .toInt(retryMaxTimeStr, 0);
                        if (retryMaxTime < 0)
                        {
                            retryMaxTime = 0;
                        }
                ...
                commandParams.put(MAX_DURATION_OF_RETRIES, Integer
                        .toString(retryMaxTime));
                commandParams.put(COMMAND_RETRY_ENABLED, Boolean
                        .toString(retryEnabled));
                ClusterVersionEntity currentClusterVersion = cluster
                        .getCurrentClusterVersion();
                if (currentClusterVersion != null)
                {
                    commandParams.put(VERSION, currentClusterVersion
                            .getRepositoryVersion().getVersion());
                }
                if (script.getTimeout() > 0)
                {
                    scriptCommandTimeout = String
                            .valueOf(script.getTimeout());
                }
            }
            else
            {
                String message = String
                        .format("Component %s of service %s has no " +
        ...
        commandParams.put(SERVICE_PACKAGE_FOLDER, serviceInfo
                .getServicePackageFolder());
        commandParams.put(HOOKS_FOLDER, stackInfo.getStackHooksFolder());
        
        String clusterName = cluster.getClusterName();
        if (customCommandExecutionHelper
                .isTopologyRefreshRequired(roleCommand
                        .name(), clusterName, serviceName))
        {
            commandParams
                    .put(ExecutionCommand.KeyNames.REFRESH_TOPOLOGY, "True");
        }
        
        execCmd.setCommandParams(commandParams);
        
        String repoInfo = customCommandExecutionHelper
                .getRepoInfo(cluster, host);
        if (LOG.isDebugEnabled())
        {
            LOG.debug("Sending repo information to agent" + ", hostname=" +
                    scHost.getHostName() + ", clusterName=" + clusterName +
                    ", stackInfo=" + stackId.getStackId() + ", repoInfo=" +
                    repoInfo);
        }
        ...
        Set<String> userSet = configHelper
                .getPropertyValuesWithPropertyType(stackId, PropertyType.USER, cluster);
        String userList = gson.toJson(userSet);
        hostParams.put(USER_LIST, userList);
        
        Set<String> groupSet = configHelper
                .getPropertyValuesWithPropertyType(stackId, PropertyType.GROUP, cluster);
        ...
        List<String> clientsToUpdateConfigsList = componentInfo
                .getClientsToUpdateConfigs();
        ...
        String clientsToUpdateConfigs = gson
                .toJson(clientsToUpdateConfigsList);
        ...
        if ((execCmd != null) && (execCmd.getConfigurationTags()
                                         .containsKey("cluster-env")))
        {
            LOG.info("AmbariManagementControllerImpl.createHostAction: created ExecutionCommand for host {}, role {}, roleCommand {}, and command ID {}, with cluster-env tags {}", execCmd
                    .getHostname(), execCmd.getRole(), execCmd
                    .getRoleCommand(), execCmd.getCommandId(), execCmd
                    .getConfigurationTags().get("cluster-env").get("tag"));
        }
    }
     ...
     */
    protected ServiceOsSpecific populateServicePackagesInfo(
            ServiceInfo serviceInfo, Map<String, String> hostParams,
            String osFamily) {
        ServiceOsSpecific hostOs = new ServiceOsSpecific(osFamily);
        List<ServiceOsSpecific> foundOSSpecifics = getOSSpecificsByFamily(serviceInfo
                .getOsSpecifics(), osFamily);
    ...
    @Override
    public List<ServiceOsSpecific.Package> getPackagesForServiceHost(
            ServiceInfo serviceInfo, Map<String, String> hostParams,
            String osFamily) {
        // Write down os specific info for the service
        ServiceOsSpecific anyOs = null;
        if (serviceInfo.getOsSpecifics()
                       .containsKey(AmbariMetaInfo.ANY_OS))
        {
            anyOs = serviceInfo.getOsSpecifics()
                               .get(AmbariMetaInfo.ANY_OS);
        }
    ...
    private List<ServiceOsSpecific> getOSSpecificsByFamily(
            Map<String, ServiceOsSpecific> osSpecifics, String osFamily) {
        List<ServiceOsSpecific> foundedOSSpecifics = new ArrayList<ServiceOsSpecific>();
        for (Entry<String, ServiceOsSpecific> osSpecific : osSpecifics
                .entrySet())
        {
    ...
    private ActionExecutionContext getActionExecutionContext(
        ...
        RequestOperationLevel operationLevel = actionRequest
                .getOperationLevel();
        if (actionRequest.isCommand())
        {
            ActionExecutionContext actionExecutionContext = new ActionExecutionContext(actionRequest
                    .getClusterName(), actionRequest
                    .getCommandName(), actionRequest
                    .getResourceFilters(), actionRequest.getParameters());
        ...
        { // If action
            ...
            ActionDefinition actionDef = ambariMetaInfo
                    .getActionDefinition(actionRequest.getActionName());
            
            if (actionDef == null)
            {
                throw new AmbariException(
                        "Action " + actionRequest.getActionName() +
                                " does not exist");
            }
            ...
            ActionExecutionContext actionExecutionContext = new ActionExecutionContext(actionRequest
                    ...
                    .getResourceFilters(), actionRequest
                    .getParameters(), actionDef.getTargetType(), actionDef
                    .getDefaultTimeout(), actionDef
                    .getTargetService(), actionDef.getTargetComponent());
            actionExecutionContext.setOperationLevel(operationLevel);
            return actionExecutionContext;
        }
    }
    
    private RequestStageContainer doStageCreation(
            RequestStageContainer requestStages, Cluster cluster,
            Map<State, List<Service>> changedServices,
        ...
        if (!changedScHosts.isEmpty() || !smokeTestServices.isEmpty())
        {
            ...
            Map<String, Set<String>> clusterHostInfo = StageUtils
                    .getClusterHostInfo(cluster);
            
            String clusterHostInfoJson = StageUtils.getGson()
                                                   .toJson(clusterHostInfo);
            String hostParamsJson = StageUtils.getGson()
                                              .toJson(customCommandExecutionHelper
                                                      .createDefaultHostParams(cluster));
            
            Stage stage = createNewStage(requestStages
                    .getLastStageId(), cluster, requestStages
                    .getId(), requestProperties
                    .get(REQUEST_CONTEXT_PROPERTY), clusterHostInfoJson, "{}", hostParamsJson);
            ...
            {
                for (State newState : changedScHosts.get(compName)
                                                    .keySet())
                {
                    for (ServiceComponentHost scHost : changedScHosts
                            .get(compName).get(newState))
                    {
                        
                        // Do not create role command for hosts that are not responding
                        if (scHost.getHostState()
                                  .equals(HostState.HEARTBEAT_LOST))
                        {
                            LOG.info(
                                    "Command is not created for servicecomponenthost " +
                                            ", clusterName=" +
                        ...
                        switch (newState)
                        {
                            case INSTALLED:
                                                ...
                                                State.INSTALL_FAILED)
                                {
                                    ...
                                    event = new ServiceComponentHostInstallEvent(scHost
                                            .getServiceComponentName(), scHost
                                            .getHostName(), nowTimestamp, scHost
                                    ...
                                    if ((oldSchState == State.INIT ||
                                            oldSchState ==
                                                    State.INSTALL_FAILED) &&
                                            kerberosHelper
                                                    .isClusterKerberosEnabled(cluster))
                                    {
                                        try
                                        {
                                            kerberosHelper
                                                    .configureService(cluster, scHost);
                                        }
                                        ...
                                        componentsToEnableKerberos
                                                .add(scHost);
                                        
                                        if (Service.Type.KERBEROS.name()
                                                                 .equalsIgnoreCase(scHost
                                                                         .getServiceName()) &&
                                        ...
                                        || oldSchState == State.STOPPING)
                                {
                                    ...
                                    event = new ServiceComponentHostStopEvent(scHost
                                            .getServiceComponentName(), scHost
                                            .getHostName(), nowTimestamp);
                                }
                                else if (oldSchState == State.UPGRADING)
                                {
                                    roleCommand = RoleCommand.UPGRADE;
                                    event = new ServiceComponentHostUpgradeEvent(scHost
                                            .getServiceComponentName(), scHost
                                            .getHostName(), nowTimestamp, scHost
                                            .getDesiredStackVersion()
                                            .getStackId());
                                ...
                                break;
                            case STARTED:
                                StackId stackId = scHost
                                        .getDesiredStackVersion();
                                ComponentInfo compInfo = ambariMetaInfo
                                        .getComponent(stackId
                                                .getStackName(), stackId
                                        ...
                                        true)
                                {
                                    ...
                                    event = new ServiceComponentHostStartEvent(scHost
                                            .getServiceComponentName(), scHost
                                            .getHostName(), nowTimestamp);
                                }
                                else
                            ...
                            case INIT:
                                throw new AmbariException(
                                        "Unsupported transition to INIT for" +
                                                " servicecomponenthost" +
                                                ", clusterName=" +
                                                cluster.getClusterName() +
                                                ", clusterId=" +
                            ...
                            default:
                                throw new AmbariException(
                                        "Unsupported state change operation" +
                                                ", newState=" +
                                                newState.toString());
                        }
                        ...
                        String keyName = scHost.getServiceComponentName()
                                               .toLowerCase();
                        if (requestProperties.containsKey(keyName))
                        {
                            ...
                            requestParameters
                                    .put(keyName, requestProperties
                                            .get(keyName));
                        }
                        
                        if (requestProperties
                                .containsKey(CLUSTER_PHASE_PROPERTY))
                        {
                            if (null == requestParameters)
                            {
                                requestParameters = new HashMap<String, String>();
                            ...
                            requestParameters
                                    .put(CLUSTER_PHASE_PROPERTY, requestProperties
                                            .get(CLUSTER_PHASE_PROPERTY));
                        }
                        ...
                        Map<String, Map<String, String>> configTags = findConfigurationTagsWithOverrides(cluster, host
                                .getHostName());
                        ...
                        {
                            if (configTags
                                    .get(Configuration.GLOBAL_CONFIG_TAG) !=
                                    null)
                            {
                                configHelper
                                        .applyCustomConfig(configurations, Configuration.GLOBAL_CONFIG_TAG, Configuration.RCA_ENABLED_PROPERTY, "false", false);
            ...
            { // Creates smoke test commands
                ...
                String smokeTestRole = actionMetadata
                        .getServiceCheckAction(serviceName);
                
                if (clientHost == null || smokeTestRole == null)
                {
                    LOG.info(
                            "Nothing to do for service check as could not find role or" +
                ...
                customCommandExecutionHelper
                        .addServiceCheckAction(stage, clientHost, smokeTestRole, nowTimestamp, serviceName, componentName, null, false, false);
            }
            ...
            if (!componentsToEnableKerberos.isEmpty())
            {
                ...
                {
                    String serviceName = scHost.getServiceName();
                    Collection<String> componentFilter = serviceFilter
                            .get(serviceName);
                ...
                {
                    kerberosHelper
                            .ensureIdentities(cluster, serviceFilter, null, hostsToForceKerberosOperations, requestStages, kerberosHelper
                                    .getManageIdentitiesDirective(requestProperties));
                }
                catch (KerberosOperationException e)
                {
            ...
            LOG.debug("Created {} stages", ((stages != null) ?
                    stages.size() : 0));
    ...
    public ExecutionCommand getExecutionCommand(Cluster cluster,
        ...
        String clusterHostInfoJson = StageUtils.getGson()
                                               .toJson(clusterHostInfo);
        Map<String, String> hostParamsCmd = customCommandExecutionHelper
                .createDefaultHostParams(cluster);
        Stage stage = createNewStage(0, cluster, 1, "", clusterHostInfoJson, "{}", "");
        
        
        Map<String, Map<String, String>> configTags = configHelper
                .getEffectiveDesiredTags(cluster, scHost.getHostName());
        Map<String, Map<String, String>> configurations = configHelper
                .getEffectiveConfigProperties(cluster, configTags);
        
        Map<String, Map<String, Map<String, String>>> configurationAttributes = new TreeMap<String, Map<String, Map<String, String>>>();
        
        createHostAction(cluster, stage, scHost, configurations, configurationAttributes, configTags, roleCommand, null, null);
        ExecutionCommand ec = stage.getExecutionCommands()
                                   .get(scHost.getHostName()).get(0)
                                   .getExecutionCommand();
        ...
        if (ec.getRole().equals(Role.HIVE_CLIENT.toString()) &&
                ec.getConfigurations()
                  .containsKey(Configuration.HIVE_CONFIG_TAG))
        {
            ec.getConfigurations().get(Configuration.HIVE_CONFIG_TAG)
              .remove(Configuration.HIVE_METASTORE_PASSWORD_PROPERTY);
        }
        
        // Add attributes
        Map<String, Map<String, Map<String, String>>> configAttributes = configHelper
                .getEffectiveConfigAttributes(cluster, ec
                        .getConfigurationTags());
        
        for (Map.Entry<String, Map<String, Map<String, String>>> attributesOccurrence : configAttributes
                .entrySet())
        {
            String type = attributesOccurrence.getKey();
            Map<String, Map<String, String>> attributes = attributesOccurrence
                    .getValue();
            ...
            if (ec.getConfigurationAttributes() != null)
            {
                ...
                {
                    ec.getConfigurationAttributes()
                      .put(type, new TreeMap<String, Map<String, String>>());
                }
                configHelper.cloneAttributesMap(attributes, ec
                        .getConfigurationAttributes().get(type));
            }
        }
    ...
    private Set<StackConfigurationDependencyResponse> getStackConfigurationDependencies(
            StackConfigurationDependencyRequest request) throws AmbariException {
        Set<StackConfigurationDependencyResponse> response = new HashSet<StackConfigurationDependencyResponse>();
        ...
        Set<PropertyInfo> properties = ambariMetaInfo
                .getPropertiesByName(stackName, stackVersion, serviceName, propertyName);
        
        for (PropertyInfo property : properties)
        {
            for (PropertyDependencyInfo dependency : property
                    .getDependedByProperties())
            {
                if (dependencyName == null ||
                        dependency.getName().equals(dependencyName))
                {
    ...
    void updateServiceStates(Cluster cluster,
                             Map<State, List<Service>> changedServices,
                             Map<State, List<ServiceComponent>> changedComps,
                             Map<String, Map<State, List<ServiceComponentHost>>> changedScHosts,
        ...
        try
        {
            ...
            {
                for (Entry<State, List<ServiceComponent>> entry : changedComps
                        .entrySet())
                {
                    State newState = entry.getKey();
    ...
    @Override
    public RequestStageContainer addStages(
            RequestStageContainer requestStages, Cluster cluster,
            Map<String, String> requestProperties,
            Map<String, String> requestParameters,
            Map<State, List<Service>> changedServices,
        ...
        {
            requestStages = new RequestStageContainer(actionManager
                    .getNextRequestId(), null, requestFactory, actionManager);
        }
    ...
    public void validateServiceComponentHostRequest(
        ...
        {
            throw new IllegalArgumentException(
                    "Property adminState cannot be modified through update. Use service " +
                            "specific DECOMMISSION action to decommision/recommission components.");
        }
    }
    
    @Override
    public String findServiceName(Cluster cluster,
                                  String componentName) throws AmbariException {
        StackId stackId = cluster.getDesiredStackVersion();
        String serviceName = ambariMetaInfo
                .getComponentToService(stackId.getStackName(), stackId
                        .getStackVersion(), componentName);
        if (LOG.isDebugEnabled())
        {
            LOG.debug("Looking up service name for component" +
                    ", componentName=" + componentName + ", serviceName=" +
                    serviceName);
        }
        ...
        {
            throw new AmbariException(
                    "Could not find service for component" +
    ...
    public synchronized void updateUsers(
        ...
        for (UserRequest request : requests)
        {
            ...
            if (null != request.getOldPassword() &&
                    null != request.getPassword())
            {
                users.modifyPassword(u.getUserName(), request
                        .getOldPassword(), request.getPassword());
            }
    ...
    public RequestStatusResponse deleteHostComponents(
        ...
        for (ServiceComponentHostRequest request : requests)
        {
            if (null == request.getComponentName())
            {
                ...
                for (ServiceComponentHost sch : cluster
                        .getServiceComponentHosts(request.getHostname()))
                {
                    ServiceComponentHostRequest schr = new ServiceComponentHostRequest(request
                            .getClusterName(), sch.getServiceName(), sch
                            .getServiceComponentName(), sch
                            .getHostName(), null);
                    expanded.add(schr);
                }
            }
        ...
        for (ServiceComponentHostRequest request : expanded)
        {
            ...
            ServiceComponent component = service
                    .getServiceComponent(request.getComponentName());
            ServiceComponentHost componentHost = component
                    .getServiceComponentHost(request.getHostname());
            ...
            {
                throw new AmbariException(
                        "To remove master or slave components they must be in " +
            ...
            {
                safeToRemoveSCHs
                        .put(component, new HashSet<ServiceComponentHost>());
            }
            safeToRemoveSCHs.get(component).add(componentHost);
        }
        
        for (Entry<ServiceComponent, Set<ServiceComponentHost>> entry : safeToRemoveSCHs
                .entrySet())
        {
            for (ServiceComponentHost componentHost : entry.getValue())
            {
                ...
                String slave_component_name = componentHost
                        .getServiceComponentName();
                HostComponentAdminState desiredAdminState = componentHost
                        .getComponentAdminState();
                State slaveState = componentHost.getState();
                //Delete hostcomponents
                entry.getKey().deleteServiceComponentHosts(componentHost
                        .getHostName());
                // If deleted hostcomponents support decomission and were decommited and stopped
                if (AmbariCustomCommandExecutionHelper.masterToSlaveMappingForDecom
                        .containsValue(slave_component_name) &&
                        desiredAdminState
                                .equals(HostComponentAdminState.DECOMMISSIONED) &&
                        slaveState.equals(State.INSTALLED))
                {
                    
                    for (Entry<String, String> entrySet : AmbariCustomCommandExecutionHelper.masterToSlaveMappingForDecom
                            .entrySet())
                    {
                        if (entrySet.getValue()
                                    .equals(slave_component_name))
                        {
                    ...
                    if (!serviceName.equals(Service.Type.HBASE.toString()))
                    {
                        ...
                        ExecuteActionRequest actionRequest = new ExecuteActionRequest(entry
                                .getKey()
                                .getClusterName(), AmbariCustomCommandExecutionHelper.DECOMMISSION_COMMAND_NAME, null, Collections
                                .singletonList(resourceFilter), null, params, true);
                        //Send request
                    ...
                    Cluster cluster = clusters
                            .getCluster(entry.getKey().getClusterName());
                    Service service = cluster.getService(serviceName);
                    ServiceComponent sc = service
                            .getServiceComponent(master_component_name);
    ...
    public void deleteUsers(
        ...
        for (UserRequest r : requests)
        {
            ...
            {
                LOG.debug(
                        "Received a delete user request" + ", username=" +
                                r.getUsername());
            }
    ...
    public void deleteGroups(
        ...
        for (GroupRequest request : requests)
        {
            LOG.debug("Received a delete group request, groupname=" +
                    request.getGroupName());
            final Group group = users.getGroup(request.getGroupName());
            if (group != null)
            {
    ...
    public void deleteMembers(
            java.util.Set<MemberRequest> requests) throws AmbariException {
        for (MemberRequest request : requests)
        {
            LOG.debug("Received a delete member request, " + request);
            users.removeMemberFromGroup(request.getGroupName(), request
                    .getUserName());
        }
    }
    ...
    RequestStatusResponse getRequestStatusResponse(long requestId) {
        ...
        List<HostRoleCommand> hostRoleCommands = actionManager
                .getRequestTasks(requestId);
        
        response.setRequestContext(actionManager
    ...
    @Override
    public Set<TaskStatusResponse> getTaskStatus(
            Set<TaskStatusRequest> requests) throws AmbariException {
        ...
        for (HostRoleCommand command : actionManager
                .getTasksByRequestAndTaskIds(requestIds, taskIds))
        {
    ...
    @Override
    public Set<ClusterResponse> getClusters(
            Set<ClusterRequest> requests) throws AmbariException {
        Set<ClusterResponse> response = new HashSet<ClusterResponse>();
        for (ClusterRequest request : requests)
        {
            try
    ...
    public Set<ServiceComponentHostResponse> getHostComponents(
        ...
        for (ServiceComponentHostRequest request : requests)
        {
            ...
            catch (ParentObjectNotFoundException e)
            {
                ...
                if (requests.size() > 1 && HostNotFoundException.class
    ...
    @Override
    public Set<ServiceConfigVersionResponse> getServiceConfigVersions(
            Set<ServiceConfigVersionRequest> requests) throws AmbariException {
    ...
    private Set<ServiceConfigVersionResponse> getServiceConfigVersions(
            ServiceConfigVersionRequest request) throws AmbariException {
        if (request.getClusterName() == null)
        {
            throw new IllegalArgumentException(
                    "Invalid arguments, cluster name" +
        ...
        for (ServiceConfigVersionResponse response : cluster
                .getServiceConfigVersions())
        {
            if (request.getServiceName() != null && !StringUtils
                    .equals(request.getServiceName(), response
                            .getServiceName()))
            {
                continue;
            }
            if (request.getVersion() != null && NumberUtils
                    .compare(request.getVersion(), response
                            .getVersion()) != 0)
            {
                continue;
            }
            if (request.getUserName() != null && !StringUtils
                    .equals(request.getUserName(), response.getUserName()))
            {
                continue;
            }
            result.add(response);
    ...
    public Set<UserResponse> getUsers(
        ...
        for (UserRequest r : requests)
        {
            ...
            {
                LOG.debug(
                        "Received a getUsers request" + ", userRequest=" +
                                r.toString());
            }
            // get them all
            if (null == r.getUsername())
            {
                ...
                {
                    UserResponse resp = new UserResponse(u.getUserName(), u
                            .isLdapUser(), u.isActive(), u.isAdmin());
            ...
            else
            {
                ...
                if (null == u)
                {
                    if (requests.size() == 1)
                    {
                        ...
                        // if there are multiple requests, this indicates an OR predicate
                        throw new ObjectNotFoundException(
                                "Cannot find user '" + r.getUsername() +
                                        "'");
                    }
                }
                else
    ...
    @Override
    public Set<GroupResponse> getGroups(
            Set<GroupRequest> requests) throws AmbariException {
        ...
        for (GroupRequest request : requests)
        {
            ...
            if (null == request.getGroupName())
            {
                ...
                {
                    final GroupResponse response = new GroupResponse(group
                            .getGroupName(), group.isLdapGroup());
                    responses.add(response);
                }
            }
            else
    ...
    protected String getClientHostForRunningAction(Cluster cluster,
                                                   Service service,
                                                   ServiceComponent serviceComponent) throws AmbariException {
        if (serviceComponent != null &&
                !serviceComponent.getServiceComponentHosts().isEmpty())
        {
            Set<String> candidateHosts = serviceComponent
                    .getServiceComponentHosts().keySet();
    ...
    protected ServiceComponent getClientComponentForRunningAction(
            Cluster cluster, Service service) throws AmbariException {
    /*
     ...
     */
        ...
        ComponentInfo compInfo = ambariMetaInfo
                .getService(stackId.getStackName(), stackId
                        .getStackVersion(), service.getName())
                .getClientComponent();
        if (compInfo != null)
        {
            try
            {
                ServiceComponent serviceComponent = service
                        .getServiceComponent(compInfo.getName());
            ...
            {
                LOG.warn(
                        "Could not find required component to run action" +
                                ", clusterName=" +
                                cluster.getClusterName() +
        ...
        Map<String, ServiceComponent> components = service
                .getServiceComponents();
     ...
     * state status.
     */
    protected void filterHostsForAction(Set<String> candidateHosts,
                                        Service service,
                                        final Cluster cluster,
                                        final Resource.Type level) throws AmbariException {
        Set<String> ignoredHosts = maintenanceStateHelper
                .filterHostsInMaintenanceState(candidateHosts, new MaintenanceStateHelper.HostPredicate()
                {
                    @Override
                    public boolean shouldHostBeRemoved(
                            final String hostname) throws AmbariException {
                        Host host = clusters.getHost(hostname);
                        return !maintenanceStateHelper
                                .isOperationAllowed(host, cluster
                                        .getClusterId(), level);
                    }
                });
        LOG.debug(
                "Ignoring hosts when selecting available hosts for action" +
                        " due to maintenance state." +
                        "Ignored hosts =" + ignoredHosts + ", cluster=" +
                        cluster.getClusterName() + ", service=" +
                        service.getName());
    }
    
    
    @Override
    public String getHealthyHost(
            Set<String> hostList) throws AmbariException {
    ...
    @Override
    public RequestStatusResponse createAction(
        ...
        {
            requestContext = requestProperties
                    .get(REQUEST_CONTEXT_PROPERTY);
        ...
        ExecuteCommandJson jsons = customCommandExecutionHelper
                .getCommandJson(actionExecContext, cluster);
        ...
        boolean kerberosServiceCheck = Role.KERBEROS_SERVICE_CHECK.name()
                                                                  .equals(actionRequest
                                                                          .getCommandName());
        if (kerberosServiceCheck)
        {
            // Parse the command parameters into a map so that additional values may be added to it
            Map<String, String> commandParamsStage = gson
                    .fromJson(commandParamsForStage, new TypeToken<Map<String, String>>()
                    {
                    }.getType());
            ...
            {
                requestStageContainer = kerberosHelper
                        .createTestIdentity(cluster, commandParamsStage, requestStageContainer);
            }
            catch (KerberosOperationException e)
            {
                throw new IllegalArgumentException(e.getMessage(), e);
        ...
        Stage stage = createNewStage(requestStageContainer
                .getLastStageId(), cluster, requestId, requestContext, jsons
                .getClusterHostInfo(), commandParamsForStage, jsons
                .getHostParamsForStage());
        ...
        {
            actionExecutionHelper
                    .addExecutionCommandsToStage(actionExecContext, stage);
        }
        ...
        if (kerberosServiceCheck)
        {
            ...
            {
                requestStageContainer = kerberosHelper
                        .deleteTestIdentity(cluster, commandParamsStage, requestStageContainer);
            }
            catch (KerberosOperationException e)
            {
                throw new IllegalArgumentException(e.getMessage(), e);
    ...
    @Override
    public Set<StackResponse> getStacks(
            Set<StackRequest> requests) throws AmbariException {
    ...
    private Set<StackResponse> getStacks(
            StackRequest request) throws AmbariException {
        ...
        {
            Collection<StackInfo> supportedStacks = ambariMetaInfo
                    .getStacks();
    ...
    public synchronized RequestStatusResponse updateStacks() throws AmbariException {
        ...
        {
            throw new AmbariException("Ambari Meta Information can't be read from the stack root directory");
        }
    ...
    @Override
    public Set<RepositoryResponse> getRepositories(
            Set<RepositoryRequest> requests) throws AmbariException {
    ...
    private Set<RepositoryResponse> getRepositories(
            RepositoryRequest request) throws AmbariException {
        ...
        {
            final RepositoryVersionEntity repositoryVersion = repositoryVersionDAO
                    .findByPK(repositoryVersionId);
            if (repositoryVersion != null)
            {
                for (OperatingSystemEntity operatingSystem : repositoryVersion
                        .getOperatingSystems())
                {
                    if (operatingSystem.getOsType().equals(osType))
                    {
                        for (RepositoryEntity repository : operatingSystem
                                .getRepositories())
                        {
                            final RepositoryResponse response = new RepositoryResponse(repository
                                    .getBaseUrl(), osType, repository
                                    .getRepositoryId(), repository
                                    .getName(), "", "", "");
                            response.setRepositoryVersionId(repositoryVersionId);
                            response.setStackName(repositoryVersion
                                    .getStackName());
                            response.setStackVersion(repositoryVersion
                                    .getStackVersion());
                            responses.add(response);
                        }
                        break;
        ...
        else
        {
            if (repoId == null)
            {
                List<RepositoryInfo> repositories = ambariMetaInfo
                        .getRepositories(stackName, stackVersion, osType);
            ...
            {
                RepositoryInfo repository = ambariMetaInfo
                        .getRepository(stackName, stackVersion, osType, repoId);
                responses = Collections
                        .singleton(repository.convertToResponse());
            }
        }
    ...
    public void updateRepositories(
        ...
        for (RepositoryRequest rr : requests)
        {
            ...
            if (null == rr.getStackVersion() ||
                    rr.getStackVersion().isEmpty())
            {
            ...
            if (null != rr.getBaseUrl())
            {
                ...
                ambariMetaInfo.updateRepoBaseURL(rr.getStackName(), rr
                        .getStackVersion(), rr.getOsType(), rr
                        .getRepoId(), rr.getBaseUrl());
            }
        }
    }
    ...
    public void verifyRepositories(
        ...
        for (RepositoryRequest request : requests)
        {
            ...
            {
                throw new AmbariException(
                        "Base url is missing for request " + request);
            }
    ...
    private void verifyRepository(
        ...
        RepositoryInfo repositoryInfo = ambariMetaInfo
                .getRepository(request.getStackName(), request
                        .getStackVersion(), request.getOsType(), request
                        .getRepoId());
        String repoName = repositoryInfo.getRepoName();
        
        String errorMessage = null;
        
        String[] suffixes = configs
                .getRepoValidationSuffixes(request.getOsType());
        for (String suffix : suffixes)
        {
            ...
            if (spec.toLowerCase().startsWith(FILE_SCHEME))
            {
                ...
                {
                    errorMessage =
                            "Could not access base url . " + spec + " . ";
                    break;
                }
            ...
            else
            {
                ...
                {
                    errorMessage = "Could not access base url . " +
                            request.getBaseUrl() + " . ";
                    if (LOG.isDebugEnabled())
                    {
                        errorMessage += ioe;
    ...
    private Set<StackVersionResponse> getStackVersions(
            StackVersionRequest request) throws AmbariException {
        ...
        {
            StackInfo stackInfo = ambariMetaInfo
                    .getStack(stackName, stackVersion);
            response = Collections
                    .singleton(stackInfo.convertToResponse());
        }
        else
        {
            try
            {
                Collection<StackInfo> stackInfos = ambariMetaInfo
                        .getStacks(stackName);
    ...
    private Set<StackServiceResponse> getStackServices(
            StackServiceRequest request) throws AmbariException {
        ...
        if (serviceName != null)
        {
            ServiceInfo service = ambariMetaInfo
                    .getService(stackName, stackVersion, serviceName);
            response = Collections
                    .singleton(new StackServiceResponse(service));
        }
        else
        {
            Map<String, ServiceInfo> services = ambariMetaInfo
                    .getServices(stackName, stackVersion);
    ...
    private Set<StackConfigurationResponse> getStackLevelConfigurations(
        ...
        {
            properties = ambariMetaInfo
                    .getStackPropertiesByName(stackName, stackVersion, propertyName);
        }
        else
        {
            properties = ambariMetaInfo
                    .getStackProperties(stackName, stackVersion);
        }
        for (PropertyInfo property : properties)
        {
    ...
    private Set<StackConfigurationResponse> getStackConfigurations(
        ...
        {
            properties = ambariMetaInfo
                    .getServiceProperties(stackName, stackVersion, serviceName);
        }
        for (PropertyInfo property : properties)
        {
            response.add(property.convertToResponse());
    ...
    private Set<StackServiceComponentResponse> getStackComponents(
        ...
        {
            ComponentInfo component = ambariMetaInfo
                    .getComponent(stackName, stackVersion, serviceName, componentName);
            response = Collections
                    .singleton(new StackServiceComponentResponse(component));
            
        }
        else
        {
            List<ComponentInfo> components = ambariMetaInfo
                    .getComponentsByService(stackName, stackVersion, serviceName);
    ...
    private Set<OperatingSystemResponse> getOperatingSystems(
        ...
        if (repositoryVersionId != null)
        {
            ...
            if (repositoryVersion != null)
            {
                for (OperatingSystemEntity operatingSystem : repositoryVersion
                        .getOperatingSystems())
                {
                    final OperatingSystemResponse response = new OperatingSystemResponse(operatingSystem
                            .getOsType());
                    response.setRepositoryVersionId(repositoryVersionId);
                    response.setStackName(repositoryVersion
                            .getStackName());
        ...
        else
        {
            if (osType != null)
            {
                OperatingSystemInfo operatingSystem = ambariMetaInfo
                        .getOperatingSystem(stackName, stackVersion, osType);
                responses = Collections
                        .singleton(operatingSystem.convertToResponse());
            }
            else
            {
                Set<OperatingSystemInfo> operatingSystems = ambariMetaInfo
                        .getOperatingSystems(stackName, stackVersion);
    ...
    public String getAuthName() {
        return AuthorizationHelper
                .getAuthenticatedName(configs.getAnonymousAuditName());
    }
    ...
    private Set<RootServiceResponse> getRootServices(
            RootServiceRequest request) throws AmbariException {
    ...
    public Map<String, String> getRcaParameters() {
        ...
        {
            url = url.replace(Configuration.HOSTNAME_MACRO, hostsMap
                    .getHostMap(hostName));
        }
        ...
        rcaParameters
                .put(AMBARI_DB_RCA_DRIVER, configs.getRcaDatabaseDriver());
        rcaParameters
                .put(AMBARI_DB_RCA_USERNAME, configs.getRcaDatabaseUser());
        rcaParameters.put(AMBARI_DB_RCA_PASSWORD, configs
    ...
    public synchronized LdapBatchDto synchronizeLdapUsersAndGroups(
            LdapSyncRequest userRequest,
            LdapSyncRequest groupRequest) throws AmbariException {
        ...
        try
        {
            ...
            if (userRequest != null)
            {
                switch (userRequest.getType())
                {
                    case ALL:
                        ldapDataPopulator
                                .synchronizeAllLdapUsers(batchInfo);
                        break;
                    case EXISTING:
                        ldapDataPopulator
                                .synchronizeExistingLdapUsers(batchInfo);
                        break;
                    case SPECIFIC:
                        ldapDataPopulator.synchronizeLdapUsers(userRequest
                                .getPrincipalNames(), batchInfo);
                        break;
                }
            }
            if (groupRequest != null)
            {
                switch (groupRequest.getType())
                {
                    case ALL:
                        ldapDataPopulator
                                .synchronizeAllLdapGroups(batchInfo);
                        break;
                    case EXISTING:
                        ldapDataPopulator
                                .synchronizeExistingLdapGroups(batchInfo);
                        break;
                    case SPECIFIC:
                        ldapDataPopulator
                                .synchronizeLdapGroups(groupRequest
                                        .getPrincipalNames(), batchInfo);
                        break;
                }
            }
    ...
    @SuppressWarnings("unchecked")
    @Override
    public void initializeWidgetsAndLayouts(Cluster cluster,
        ...
        try
        {
            ...
            {
                // Service widgets
                ServiceInfo serviceInfo = stackInfo
                        .getService(service.getName());
                File widgetDescriptorFile = serviceInfo
                        ...
                        widgetDescriptorFile.exists())
                {
                    try
                    {
                        widgetDescriptor = gson
                                .fromJson(new FileReader(widgetDescriptorFile), widgetLayoutType);
                    }
                    catch (Exception ex)
                    {
            ...
            else
            {
                ...
                String widgetDescriptorFileLocation = stackInfo
                        .getWidgetsDescriptorFileLocation();
                if (widgetDescriptorFileLocation != null)
                {
                    ...
                    if (widgetDescriptorFile.exists())
                    {
                        ...
                        {
                            String msg =
                                    "Error loading widgets from file: " +
                                            widgetDescriptorFile;
                            LOG.error(msg, ex);
                            throw new AmbariException(msg);
                        }
        ...
        {
            throw new AmbariException(
                    "Error creating stack widget artifacts. " +
                            (service != null ?
    ...
    private WidgetEntity addIfNotExistsWidgetEntity(
            WidgetLayoutInfo layoutInfo, ClusterEntity clusterEntity,
            String user, long createTime) {
        List<WidgetEntity> createdEntities = widgetDAO
                .findByName(clusterEntity.getClusterId(), layoutInfo
                        .getWidgetName(), user, layoutInfo
        ...
        if (createdEntities == null || createdEntities.isEmpty())
        {
            ...
            widgetEntity
                    .setScope(WidgetResourceProvider.SCOPE.CLUSTER.name());
            widgetEntity.setWidgetName(layoutInfo.getWidgetName());
            widgetEntity.setDefaultSectionName(layoutInfo
                    .getDefaultSectionName());
            ...
            widgetEntity.setProperties(gson
                    .toJson(layoutInfo.getProperties()));
            widgetEntity
                    .setWidgetValues(gson.toJson(layoutInfo.getValues()));
            widgetEntity
                    .setListWidgetLayoutUserWidgetEntity(new LinkedList<WidgetLayoutUserWidgetEntity>());
            LOG.info("Creating cluster widget with: name = " +
                    layoutInfo.getWidgetName() + ", type = " +
                    layoutInfo.getType() + ", " +
        ...
        {
            LOG.warn(
                    "Skip creating widget from stack artifact since one or more " +
                            "already exits with name = " +
    ...
    @Transactional
    private void createWidgetsAndLayouts(Cluster cluster,
                                         List<WidgetLayout> widgetLayouts) {
        String user = "ambari";
        ...
        if (widgetLayouts != null)
        {
            ...
            {
                List<WidgetLayoutEntity> existingEntities = widgetLayoutDAO
                        .findByName(clusterId, widgetLayout
                                .getLayoutName(), user);
                ...
                if (existingEntities == null || existingEntities.isEmpty())
                {
                    ...
                    layoutEntity
                            .setLayoutName(widgetLayout.getLayoutName());
                    layoutEntity
                            .setDisplayName(widgetLayout.getDisplayName());
                    layoutEntity
                            .setSectionName(widgetLayout.getSectionName());
                    layoutEntity
                            .setScope(WidgetLayoutResourceProvider.SCOPE.CLUSTER
                                    .name());
                    ...
                    int order = 0;
                    for (WidgetLayoutInfo layoutInfo : widgetLayout
                            .getWidgetLayoutInfoList())
                    {
                        if (layoutInfo.getDefaultSectionName() == null)
                        {
                            layoutInfo.setDefaultSectionName(layoutEntity
                                    .getSectionName());
                        }
                        ...
                        if (widgetEntity != null && layoutInfo.isVisible())
                        {
                            ...
                            widgetLayoutUserWidgetEntity
                                    .setWidget(widgetEntity);
                            widgetLayoutUserWidgetEntity
                                    .setWidgetOrder(order++);
                            widgetLayoutUserWidgetEntity
                                    .setWidgetLayout(layoutEntity);
                            widgetLayoutUserWidgetEntityList
                                    .add(widgetLayoutUserWidgetEntity);
                            widgetEntity
                    ...
                    layoutEntity
                            .setListWidgetLayoutUserWidgetEntity(widgetLayoutUserWidgetEntityList);
                ...
                else
                {
                    ...
                    {
                        LOG.warn(
                                "Skip updating layout since multiple widget layouts " +
                                        "found with: name = " +
                                        widgetLayout.getLayoutName() +
                                        ", " +
                                        "user = " + user + ", cluster = " +
                                        cluster.getClusterName());
                    }
                    ...
                    {
                        WidgetLayoutEntity existingLayoutEntity = existingEntities
                                .iterator().next();
                        existingLayoutEntity.setSectionName(widgetLayout
                                .getSectionName());
                        existingLayoutEntity.setDisplayName(widgetLayout
                                .getDisplayName());
                        // Add new widgets to end of the existing ones
                        List<WidgetLayoutUserWidgetEntity> layoutUserWidgetEntities = existingLayoutEntity
                                .getListWidgetLayoutUserWidgetEntity();
                        if (layoutUserWidgetEntities == null)
                        {
                            layoutUserWidgetEntities = new LinkedList<WidgetLayoutUserWidgetEntity>();
                            existingLayoutEntity
                                    .setListWidgetLayoutUserWidgetEntity(layoutUserWidgetEntities);
                        }
                        int order = layoutUserWidgetEntities.size() - 1;
                        List<WidgetLayoutInfo> layoutInfoList = widgetLayout
                                .getWidgetLayoutInfoList();
                        if (layoutInfoList != null &&
                                !layoutInfoList.isEmpty())
                        {
                            for (WidgetLayoutInfo layoutInfo : layoutInfoList)
                            {
                                        ...
                                        layoutInfo.isVisible())
                                {
                                    ...
                                    widgetLayoutUserWidgetEntity
                                            .setWidgetOrder(order++);
                                    widgetLayoutUserWidgetEntity
                                            .setWidgetLayout(existingLayoutEntity);
                                    layoutUserWidgetEntities
                                            .add(widgetLayoutUserWidgetEntity);
                                    widgetEntity
                                            .getListWidgetLayoutUserWidgetEntity()
                                            .add(widgetLayoutUserWidgetEntity);
                        ...
                        widgetLayoutDAO
                                .mergeWithFlush(existingLayoutEntity);
                    }
                }
            }
        }
    ...
    protected void verifyAuthorization() throws AmbariException {
        ...
        for (GrantedAuthority grantedAuthority : securityHelper
                .getCurrentAuthorities())
        {
            if (grantedAuthority instanceof AmbariGrantedAuthority)
            {
                AmbariGrantedAuthority authority = (AmbariGrantedAuthority) grantedAuthority;
                PrivilegeEntity privilegeEntity = authority
                        .getPrivilegeEntity();
                Integer permissionId = privilegeEntity.getPermission()
                                                      .getId();
                
                if (permissionId
                        .equals(PermissionEntity.AMBARI_ADMIN_PERMISSION))
                {
                    isAuthorized = true;
                    break;
                }
    ...
    public Map<String, String> getCredentialStoreServiceProperties() {
        ...
        properties.put("storage.persistent", String
                .valueOf(credentialStoreService
                        .isInitialized(CredentialStoreType.PERSISTED)));
        properties.put("storage.temporary", String
                .valueOf(credentialStoreService
                        .isInitialized(CredentialStoreType.TEMPORARY)));
        return properties;
    }
}
