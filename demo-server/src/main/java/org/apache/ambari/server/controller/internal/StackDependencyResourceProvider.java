...
public class StackDependencyResourceProvider extends AbstractResourceProvider
{
    ...
    protected static final String STACK_NAME_ID = PropertyHelper
            .getPropertyId("Dependencies", "stack_name");
    protected static final String STACK_VERSION_ID = PropertyHelper
            .getPropertyId("Dependencies", "stack_version");
    protected static final String DEPENDENT_SERVICE_NAME_ID = PropertyHelper
            .getPropertyId("Dependencies", "dependent_service_name");
    protected static final String DEPENDENT_COMPONENT_NAME_ID = PropertyHelper
            .getPropertyId("Dependencies", "dependent_component_name");
    protected static final String SERVICE_NAME_ID = PropertyHelper
            .getPropertyId("Dependencies", "service_name");
    protected static final String COMPONENT_NAME_ID = PropertyHelper
            .getPropertyId("Dependencies", "component_name");
    protected static final String SCOPE_ID = PropertyHelper
            .getPropertyId("Dependencies", "scope");
    ...
    private static Set<String> pkPropertyIds = new HashSet<String>(Arrays
            .asList(new String[]{SERVICE_NAME_ID, COMPONENT_NAME_ID}));
    ...
    public Set<Resource> getResources(Request request,
                                      Predicate predicate) throws SystemException, UnsupportedPropertyException, NoSuchResourceException, NoSuchParentResourceException {
        ...
        for (Map<String, Object> properties : requestProps)
        {
            ...
            {
                resources
                        .addAll(getDependencyResources(properties, getRequestPropertyIds(request, predicate)));
            }
    ...
    @Override
    public RequestStatus createResources(
            Request request) throws SystemException, UnsupportedPropertyException, ResourceAlreadyExistsException, NoSuchParentResourceException {
    ...
    @Override
    public RequestStatus deleteResources(
            Predicate predicate) throws SystemException, UnsupportedPropertyException, NoSuchResourceException, NoSuchParentResourceException {
     ...
     */
    private Collection<Resource> getDependencyResources(
            Map<String, Object> properties,
            Set<String> requestedIds) throws SystemException, NoSuchParentResourceException, NoSuchResourceException {

        final String stackName = (String) properties.get(STACK_NAME_ID);
        final String version = (String) properties.get(STACK_VERSION_ID);
        final String dependentService = (String) properties
                .get(DEPENDENT_SERVICE_NAME_ID);
        final String dependentComponent = (String) properties
                .get(DEPENDENT_COMPONENT_NAME_ID);
        final String dependencyName = (String) properties
                .get(COMPONENT_NAME_ID);
            ...
            dependencies.add(getResources(new Command<DependencyInfo>()
            {
                ...
                public DependencyInfo invoke() throws AmbariException {
                    return ambariMetaInfo
                            .getComponentDependency(stackName, version, dependentService, dependentComponent, dependencyName);
                }
            }));
        }
        else
        {
            dependencies
                    .addAll(getResources(new Command<List<DependencyInfo>>()
                    {
                        @Override
                        public List<DependencyInfo> invoke() throws AmbariException {
                            return ambariMetaInfo
                                    .getComponentDependencies(stackName, version, dependentService, dependentComponent);
                        }
                    }));
        }
        ...
        for (DependencyInfo dependency : dependencies)
        {
            ...
            {
                resources
                        .add(toResource(dependency, stackName, version, dependentService, dependentComponent, requestedIds));
            }
        }
        return resources;
    }
     ...
     */
    private Resource toResource(DependencyInfo dependency,
                                String stackName, String version,
                                String dependentService,
                                String dependentComponent,
                                Set<String> requestedIds) {

        Resource resource = new ResourceImpl(Resource.Type.StackServiceComponentDependency);

        setResourceProperty(resource, SERVICE_NAME_ID, dependency
                .getServiceName(), requestedIds);
        setResourceProperty(resource, COMPONENT_NAME_ID, dependency
                .getComponentName(), requestedIds);
        ...
        setResourceProperty(resource, SCOPE_ID, dependency
                .getScope(), requestedIds);

        AutoDeployInfo autoDeployInfo = dependency.getAutoDeploy();
        if (autoDeployInfo != null)
        {
            setResourceProperty(resource, AUTO_DEPLOY_ENABLED_ID, autoDeployInfo
                    .isEnabled(), requestedIds);

            if (autoDeployInfo.getCoLocate() != null)
            {
                setResourceProperty(resource, AUTO_DEPLOY_LOCATION_ID, autoDeployInfo
                        .getCoLocate(), requestedIds);
            }
        }
        return resource;
    }
