 ...
 */
public abstract class AbstractProviderModule implements ProviderModule, ResourceProviderObserver, JMXHostProvider
{
    ...
    private static final String CLUSTER_NAME_PROPERTY_ID = PropertyHelper
            .getPropertyId("Clusters", "cluster_name");
    private static final String HOST_COMPONENT_CLUSTER_NAME_PROPERTY_ID = PropertyHelper
            .getPropertyId("HostRoles", "cluster_name");
    private static final String HOST_COMPONENT_HOST_NAME_PROPERTY_ID = PropertyHelper
            .getPropertyId("HostRoles", "host_name");
    private static final String HOST_COMPONENT_COMPONENT_NAME_PROPERTY_ID = PropertyHelper
    ...
    static
    {
        ...
        initPropMap.put("DATANODE", new String[]{
                "dfs.datanode.http" + ".address"});
        initPropMap.put("NAMENODE-HTTPS", new String[]{
                "dfs.namenode" + ".https-address", "dfs.https.port"});
        serviceDesiredProperties.put(Service.Type.HDFS, initPropMap);

        initPropMap = new HashMap<String, String[]>();
        initPropMap.put("JOBTRACKER", new String[]{
                "mapred.job.tracker.http" + ".address"});
        initPropMap.put("TASKTRACKER", new String[]{
                "mapred.task.tracker" + ".http.address"});
        serviceDesiredProperties.put(Service.Type.MAPREDUCE, initPropMap);

        initPropMap = new HashMap<String, String[]>();
        initPropMap.put("HBASE_MASTER", new String[]{
                "hbase.master.info" + ".port"});
        serviceDesiredProperties.put(Service.Type.HBASE, initPropMap);

        initPropMap = new HashMap<String, String[]>();
        initPropMap.put("RESOURCEMANAGER", new String[]{
                "yarn" + ".resourcemanager.webapp.address"});
        initPropMap.put("RESOURCEMANAGER-HTTPS", new String[]{
                "yarn" + ".resourcemanager.webapp.https.address"});
        initPropMap.put("NODEMANAGER", new String[]{
                "yarn.nodemanager" + ".webapp.address"});
        serviceDesiredProperties.put(Service.Type.YARN, initPropMap);

        initPropMap = new HashMap<String, String[]>();
        initPropMap.put("HISTORYSERVER", new String[]{
        ...
        initPropMap = new HashMap<String, String[]>();
        initPropMap
                .put("RESOURCEMANAGER", new String[]{"yarn.http.policy"});
    ...
    public AbstractProviderModule() {
        ...
        //        {
        //            metricCacheProvider = managementController
    ...
    //                                       MetricsService service) throws SystemException {
    //        checkInit();
    ...
    //            try
    //            {
    //                String configType = serviceConfigTypes
    //                        .get(Service.Type.AMBARI_METRICS);
    //                String currentConfigVersion = getDesiredConfigVersion(clusterName, configType);
    //                String oldConfigVersion = serviceConfigVersions
    //                        .get(Service.Type.AMBARI_METRICS);
    //
    //                if (!currentConfigVersion.equals(oldConfigVersion))
    //                {
    //                    serviceConfigVersions
    //                            .put(Service.Type.AMBARI_METRICS, currentConfigVersion);
    //
    //                    Map<String, String> configProperties = getDesiredConfigMap(clusterName, currentConfigVersion, configType, Collections
    //                            .singletonMap("METRICS_COLLECTOR", new String[]{
    //                                    "timeline" +
    //                                            ".metrics" +
    //                                            ".service" +
    //                                            ".webapp" +
    ...
    //                    {
    //                        clusterMetricServerPort = getPortString(configProperties
    //                                .get("METRICS_COLLECTOR"));
    //                    }
    //                    else
    //                    {
    ...
    //
    //    @Override
    //    public String getHostName(String clusterName,
    //                              String componentName) throws SystemException {
    //        checkInit();
    //        return clusterHostComponentMap.get(clusterName).get(componentName);
    //    }
    //
    @Override
    public Set<String> getHostNames(String clusterName,
                                    String componentName) {
        Set<String> hosts = null;
        try
        {
            Cluster cluster = managementController.getClusters()
                                                  .getCluster(clusterName);
            String serviceName = managementController
                    .findServiceName(cluster, componentName);
            hosts = cluster.getService(serviceName)
                           .getServiceComponent(componentName)
                           .getServiceComponentHosts().keySet();
        }
        catch (Exception e)
        {
            LOG.warn("Exception in getting host names for jmx metrics: " +
                    "", e);
        }
        return hosts;
    }
    ...
    //        if (service.equals(GANGLIA))
    //        {
    //            return isHostComponentLive(clusterName, collectorHostName, "GANGLIA", Role.GANGLIA_SERVER
    //                    .name());
    //        }
    //        else if (service.equals(TIMELINE_METRICS))
    //        {
    //            return isHostComponentLive(clusterName, collectorHostName, "AMBARI_METRICS", Role.METRICS_COLLECTOR
    //                    .name());
    //        }
    //        return false;
    //    }

    private boolean isHostComponentLive(String clusterName,
                                        String hostName,
                                        String serviceName,
                                        String componentName) {
        if (clusterName == null)
        ...
        {
            LOG.debug("Error checking " + componentName +
                    " server host component state: ", e);
            return false;
        }

        //Cluster without SCH
        return componentHostResponse != null &&
                componentHostResponse.getLiveState()
                                     .equals(State.STARTED.name());
    }
    ...
    protected boolean isHostLive(String clusterName, String hostName) {
        ...
        try
        {
            HostRequest hostRequest = new HostRequest(hostName, clusterName, Collections
                    .<String, String>emptyMap());
            Set<HostResponse> hosts = HostResourceProvider
                    .getHosts(managementController, hostRequest);

            hostResponse =
                    hosts.size() == 1 ? hosts.iterator().next() : null;
        }
        catch (AmbariException e)
        {
            LOG.debug("Error checking of Ganglia server host live " +
                    "status: ", e);
            return false;
        }
        //Cluster without host
        return hostResponse != null && !hostResponse.getHostState()
    ...
    @Override
    public String getPort(String clusterName,
                          String componentName) throws SystemException {
    ...
    public String getPort(String clusterName, String componentName,
                          boolean httpsEnabled) throws SystemException {
        ...
        if (service != null)
        {
            try
            {
                String currVersion = getDesiredConfigVersion(clusterName, serviceConfigTypes
                        .get(service));
                String oldVersion = serviceConfigVersions.get(service);

                // We only update port map when a config version updates,
                // Since concurrent thread access is expected we err on
                // the side of
                // performance with a ConcurrentHashMap and maybe get
                // default/existing
                        ...
                        !clusterJmxPorts.containsKey(componentName))
                {
                    ...
                    if (httpsEnabled)
                    {
                        componentsHttpsPorts = serviceDesiredProperties
                                .get(service)
                                .get(componentName + "-HTTPS");
                    }
                    else
                    {
                        componentsHttpsPorts = serviceDesiredProperties
                                .get(service).get(componentName);
                    }
                    componentPorts
                            .put(componentName, componentsHttpsPorts);

                    Map<String, String> portMap = getDesiredConfigMap(clusterName, currVersion, serviceConfigTypes
                            .get(service), componentPorts);

                    for (Entry<String, String> entry : portMap.entrySet())
                    {
                        // portString will be null if the property
                        // defined for the component doesn't exist
                        // this will trigger using the default port for
                        // the component
                        String portString = getPortString(entry
                                .getValue());
                        if (null != portString)
                        {
                            clusterJmxPorts
                                    .put(entry.getKey(), portString);
                        }
                    }
                }
            }
    ...
    private String postProcessPropertyValue(String key, String value,
        ...
        if (value != null && key != null && value.contains("${"))
        {
            ...
            while (refValueString.contains("${"))
            {
                ...
                String valueRef = refValueString
                        .substring(startValueRef, endValueRef);
                refValueString = refValueString.substring(endValueRef + 1);
                String trueValue = postProcessPropertyValue(valueRef, properties
                        .get(valueRef), properties, prevProps);
                if (trueValue != null)
                {
    ...
    protected void createPropertyProviders(Resource.Type type) {
        ...
        ComponentSSLConfiguration configuration = ComponentSSLConfiguration
                .instance();
        ...
        //                    providers
        //                            .add(new
        // AlertSummaryPropertyProvider(type,
        ...
        //                    providers
        //                            .add
        // (createMetricsHostPropertyProvider(type,
        //                                    streamProvider,
        //                                    ComponentSSLConfiguration
        //                                            .instance(), this,
        // this,
        ...
        //                case Component:
        //                {
        //                    // TODO as we fill out stack metric
        // definitions,
        //                    // these can be phased out
        //                    PropertyProvider jpp =
        // createJMXPropertyProvider
        //                            (type, streamProvider, this, this,
        //                                    PropertyHelper
        //                                    .getPropertyId
        //
        ...
        //                    gpp =
        // createMetricsComponentPropertyProvider(type,
        //                            streamProvider,
        // ComponentSSLConfiguration
        //                                    .instance(), this, this,
        // PropertyHelper
        ...
        //                    providers
        //                            .add(new
        // StackDefinedPropertyProvider(type,
        //                                    this, this, this,
        // streamProvider,
        //                                    PropertyHelper
        //                                    .getPropertyId
        ...
        //                    PropertyProvider gpp = null;
        //                    gpp =
        // createMetricsHostComponentPropertyProvider
        //                            (type, streamProvider,
        //                                    ComponentSSLConfiguration
        //                                    .instance(), this, this,
        // PropertyHelper
        ...
        //
        //                    providers
        //                            .add(new StackDefinedPropertyProvider(type,
        //                                    this, this, this, streamProvider,
        //                                    PropertyHelper
        //                                    .getPropertyId("HostRoles",
        //                                            "cluster_name"), PropertyHelper
        //                                    .getPropertyId("HostRoles",
        //                                            "host_name"), PropertyHelper
        //                                    .getPropertyId("HostRoles",
        //                                            "component_name"),
        //                                    PropertyHelper
        //                                    .getPropertyId("HostRoles", "state")
        //                                    , jpp, gpp));
        //                }
        //                break;
        //                default:
    ...
    private void initProviderMaps() throws SystemException {
        ...
        requestInfoProperties
                .put(ClusterResourceProvider.GET_IGNORE_PERMISSIONS_PROPERTY_ID, "true");

        Request request = PropertyHelper
                .getReadRequest(propertyIds, requestInfoProperties, null, null, null);
        ...
        try
        {
            ...
            for (Resource cluster : clusters)
            {
                ...
                String clusterName = (String) cluster
                        .getPropertyValue(CLUSTER_NAME_PROPERTY_ID);

                // initialize the host component map and Ganglia server
                // from the known hosts components...
                provider = getResourceProvider(Resource.Type.HostComponent);

                request = PropertyHelper
                        .getReadRequest(HOST_COMPONENT_HOST_NAME_PROPERTY_ID, HOST_COMPONENT_COMPONENT_NAME_PROPERTY_ID);

                Predicate predicate = new PredicateBuilder()
                        .property(HOST_COMPONENT_CLUSTER_NAME_PROPERTY_ID).
                                equals(clusterName).toPredicate();

                Set<Resource> hostComponents = provider
                        .getResources(request, predicate);
                Map<String, String> hostComponentMap = clusterHostComponentMap
                        .get(clusterName);

                if (hostComponentMap == null)
                {
                    hostComponentMap = new HashMap<String, String>();
                    clusterHostComponentMap
                            .put(clusterName, hostComponentMap);
                }

                for (Resource hostComponent : hostComponents)
                {
                    String componentName = (String) hostComponent
                            .getPropertyValue(HOST_COMPONENT_COMPONENT_NAME_PROPERTY_ID);
                    String hostName = (String) hostComponent
                    ...
                    {
                        clusterGangliaCollectorMap
                                .put(clusterName, hostName);
                    }
        ...
        catch (UnsupportedPropertyException e)
        {
            ...
            {
                LOG.error("Caught UnsupportedPropertyException while " +
                        "trying to get the host mappings.", e);
            }
            throw new SystemException(
                    "An exception occurred while initializing the host " +
                            "mappings: " +
                            e, e);
        }
        catch (NoSuchResourceException e)
        {
            if (LOG.isErrorEnabled())
            {
                LOG.error("Caught NoSuchResourceException exception " +
                        "while trying to get the host mappings.", e);
            }
            throw new SystemException(
                    "An exception occurred while initializing the host " +
                            "mappings: " +
        ...
        catch (NoSuchParentResourceException e)
        {
            if (LOG.isErrorEnabled())
            {
                LOG.error("Caught NoSuchParentResourceException " +
    ...
    private String getDesiredConfigVersion(String clusterName,
                                           String configType) throws NoSuchParentResourceException, UnsupportedPropertyException, SystemException {
        ...
        Predicate basePredicate = new PredicateBuilder()
                .property(ClusterResourceProvider.CLUSTER_NAME_PROPERTY_ID)
                .equals(clusterName).toPredicate();
        ...
        try
        {
            ...
            Set<String> propertyIds = new HashSet<String>();
            propertyIds
                    .add(ClusterResourceProvider.CLUSTER_NAME_PROPERTY_ID);
            propertyIds
                    .add(ClusterResourceProvider.CLUSTER_DESIRED_CONFIGS_PROPERTY_ID);

            Map<String, String> requestInfoProperties = new HashMap<String, String>();
            requestInfoProperties
                    .put(ClusterResourceProvider.GET_IGNORE_PERMISSIONS_PROPERTY_ID, "true");

            Request readRequest = PropertyHelper
                    .getReadRequest(propertyIds, requestInfoProperties, null, null, null);

            clusterResource = clusterResourceProvider
                    .getResources(readRequest, basePredicate);
        }
        catch (NoSuchResourceException e)
        {
        ...
        if (clusterResource != null)
        {
            ...
            {
                Map<String, Object> configs = resource.getPropertiesMap()
                                                      .get(ClusterResourceProvider.CLUSTER_DESIRED_CONFIGS_PROPERTY_ID);
                if (configs != null)
                {
                    DesiredConfig config = (DesiredConfig) configs
                            .get(configType);
                    if (config != null)
                    {
    ...
    //
    //        return new JMXPropertyProvider(PropertyHelper
    //                .getJMXPropertyIds(type), streamProvider, jmxHostProvider, metricsHostProvider, clusterNamePropertyId, hostNamePropertyId, componentNamePropertyId, statePropertyId);
    //    }
    //
    //
    //    /**
    ...
    //     */
    //    private PropertyProvider createMetricsHostPropertyProvider(
    //            Resource.Type type, StreamProvider streamProvider,
    //            ComponentSSLConfiguration configuration,
    //            MetricHostProvider hostProvider,
    //            MetricsServiceProvider serviceProvider,
    //            String clusterNamePropertyId, String
    // hostNamePropertyId) {
    //        return MetricsPropertyProvider.createInstance(type,
    // PropertyHelper
    //                .getMetricPropertyIds(type), streamProvider,
    //                configuration, metricCacheProvider, hostProvider,
    //                serviceProvider, clusterNamePropertyId,
    //                hostNamePropertyId, null);
    //    }

    //    /**
    //     * Create the Ganglia component property provider for the
    // given type.
    //     */
    //    private PropertyProvider createMetricsComponentPropertyProvider(
    //            Resource.Type type, StreamProvider streamProvider,
    //            ComponentSSLConfiguration configuration,
    //            MetricHostProvider hostProvider,
    //            MetricsServiceProvider serviceProvider,
    ...
    //                .getMetricPropertyIds(type), streamProvider,
    // configuration, metricCacheProvider, hostProvider,
    // serviceProvider, clusterNamePropertyId, null,
    // componentNamePropertyId);
    //    }


    //    /**
    //     * Create the Ganglia host component property provider for the
    // given type.
    //     */
    ...
    //            MetricHostProvider hostProvider,
    //            MetricsServiceProvider serviceProvider,
    //            String clusterNamePropertyId, String hostNamePropertyId,
    //            String componentNamePropertyId) {
    //
    //        return MetricsPropertyProvider.createInstance(type,
    // PropertyHelper
    //                .getMetricPropertyIds(type), streamProvider,
    // configuration, metricCacheProvider, hostProvider,
    // serviceProvider, clusterNamePropertyId, hostNamePropertyId,
    // componentNamePropertyId);
    //    }

    @Override
    public String getJMXProtocol(String clusterName,
                                 String componentName) {
        ...
        try
        {
            if (componentName.equals("NAMENODE") ||
                    componentName.equals("RESOURCEMANAGER"))
            {
                Service.Type service = componentServiceMap
                        .get(componentName);
                String config = serviceConfigTypes.get(service);
                String newSiteConfigVersion = getDesiredConfigVersion(clusterName, config);
                String cachedSiteConfigVersion = clusterHdfsSiteConfigVersionMap
                        .get(clusterName);
                if (!newSiteConfigVersion.equals(cachedSiteConfigVersion))
                {
                    Map<String, String> protocolMap = getDesiredConfigMap(clusterName, newSiteConfigVersion, config, jmxDesiredProperties
                            .get(componentName));
                    jmxProtocolString = getJMXProtocolString(protocolMap
                            .get(componentName));
                    clusterJmxProtocolMap
                            .put(clusterName, jmxProtocolString);
                }
            }
            else
            {
        ...
        {
            LOG.info("Exception while detecting JMX protocol for " +
                    "clusterName = " +
                    clusterName +
                    ", componentName = " + componentName, e);
            LOG.info("Defaulting JMX to HTTP protocol for  for " +
                    "clusterName = " +
                    clusterName +
        ...
        {
            LOG.debug("Detected JMX protocol is null for clusterName = " +
                    clusterName +
                    ", componentName = " + componentName);
            LOG.debug("Defaulting JMX to HTTP protocol for  for " +
                    "clusterName = " +
                    clusterName +
                    ", componentName = " + componentName +
                    componentName);
        ...
        {
            LOG.debug("JMXProtocol = " + jmxProtocolString +
                    ", for clusterName=" + clusterName +
                    ", componentName = " + componentName);
        }
