...
public abstract class BaseService
{
    ...
    protected Response handleRequest(HttpHeaders headers, String body,
        ...
        try
        {
            ...
            while (iterator.hasNext() && result.getStatus().getStatus()
                                               .equals(ResultStatus.STATUS.OK))
            {
                RequestBody requestBody = iterator.next();

                Request request = getRequestFactory()
                        .createRequest(headers, requestBody, uriInfo, requestType, resource);
    ...
    protected ResultSerializer getResultSerializer(
        ...
        else if (mediaType.equals(MediaType.APPLICATION_JSON_TYPE))
        {
            return new ResultSerializer()
            {
                @Override
                public Object serialize(Result result) {
                    return JSON.parse(serializer.serialize(result)
                                                .toString());
                }

                @Override
                public Object serializeError(ResultStatus error) {
                    return JSON.parse(serializer.serializeError(error)
                                                .toString());
                }
            };
        }
        ...
        throw new IllegalArgumentException(
                "The media type " + mediaType + " is not supported.");
    }
